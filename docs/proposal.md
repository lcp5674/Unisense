...
> 版本：v1.0　|　状态：评审稿　|　最后更新：2026-08-06
> 文档定位：面向产品、技术、治理、运营各方的统一交付物，描述"指标语义中台"的产品目标、用户与场景、功能规划、架构设计、技术实现粒度、交互规格、非功能要求、验收标准、依赖与假设。

### 修订记录 [R6-09]

| 轮次 | 标记前缀 | 焦点 | 发现数 | 状态 | 累计 [R11-20] |
|------|---------|------|--------|------|------|
| R1 (Round 7) | `I1`–`I20` | 逻辑/工程可行性 | 20 | 已嵌入 | 20 |
| R2 (Round 8) | `P1`–`P20` | 产品视角 | 20 | 已嵌入 | 40 |
| R2 补 | 1.3a→1.5 | 产品前提审计 | 7 缺口 + 7 填充 | 已嵌入 | 47 |
| R2 补 | A1–A5, B1–B7, C1–C6, D1–D3 | 数仓生产场景 | 21 | 已嵌入 | 68 |
| R3 | `R3-01`–`R3-30` | 第三轮产品审查 | 30（含 2 误判） | 已嵌入 | 98 |
| R4 | `R4-01`–`R4-13` | 技术可行性·产品一致性·文档质量 | 13 | 已嵌入 | 111 |
| R5 | `R5-01`–`R5-13` | 工程落地·产品闭环·数据模型与API | 13 | 已嵌入 | 124 |
| R6 | `R6-01`–`R6-09` | 端到端可追溯性·跨模块语义一致性·文档质量 | 9 | 已嵌入 | 133 |
| R7 | `R7-01`–`R7-09` | 安全与访问控制审计 | 9 | 已嵌入 | 142 |
| R8 | `R8-01`–`R8-09` | 多存储数据一致性·运维就绪度 | 9 | 已嵌入 | 151 |
| R9 | `R9-01`–`R9-09` | 并发与竞态·API 契约完整性·配置治理 | 9 | 已嵌入 | 160 |
| R10 | `R10-01`–`R10-09` | 成本归因模型·数据血缘边界·消费方体验最后一公里 | 9 | 已嵌入 | 169 |
| R11 | `R11-01`–`R11-20` | 全量多维度系统性审查（语义一致性·数据模型·API契约·交互规格·分期交付·配置参数·文档质量） | 20 | 已嵌入 | 189 |
| R12 | `R12-01`–`R12-17` | 端到端验收闭环精审（9.1量化标准·9.2验收方法·9.4冒烟测试路径·6.5性能补项·7.1 FR引用·附录B参数） | 17 | 已嵌入 | 206 |
| R13 | `R13-01`–`R13-15` | API 契约精审（4.11.2端点参数·响应Schema·排序过滤·幂等性·批量端点·血缘查询·meta标注·错误码语义·通用Header） | 15 | 已嵌入 | 221 |
...
├──────────────────────────────────────────────────────────────────┤
│ 语义层   指标定义(4.5) · 术语库(4.6) · 血缘(4.4) · 冲突仲裁(4.7)             │
├──────────────────────────────────────────────────────────────────┤
...
> 版本号以 TD §0 技术总览表为权威源，此处标注最低版本约束。

| 层 | 技术 | 版本 | 职责 |
|----|------|------|------|
| 后端 | Python（FastAPI） | Python ≥3.11 / FastAPI ≥0.115 | 指标 API、解析调度、执行编排 |
| 前端 | React（TS） | React 19 / TypeScript strict | 指标目录、注册页、血缘图、驾驶舱 |
| 关系存储 | MySQL | 8.0 | 指标/版本/权限/审计主数据 |
| 图存储 | Neo4j | 5.x | 表级/字段级血缘、影响面分析 |
| 检索 | Elasticsearch | 8.15 | 指标/术语全文检索、同义词匹配 |
| 缓存/队列 | Redis | 7 | 会话、查询缓存、LLM 批量任务队列 |
| OLAP 引擎 | Doris / StarRocks | Doris 2.1 / StarRocks 3.3 | 指标计算下推、预聚合 |
| LLM 适配层 | 模型服务网关（不重造 LLM） | — | 统一封装现有模型服务的调用、限流、降级与审计（须兼容 OpenAI 协议接口，呼应 1.6 不重造 LLM、4.3 调用约束）；模型注册/连通性测试/健康探测/Prompt 模板管理见 4.3a |
| 密钥 | Secret Manager | — | 数据源凭证、API Secret 托管 |
| 依赖管理 | Poetry | ≥2.0 | Python 依赖锁定与版本管理（pyproject.toml + poetry.lock） |
| 日志 | structlog | ≥24.0 | 结构化 JSON 日志 |
| 可观测性 | OpenTelemetry | ≥1.27 | 分布式追踪（trace_id 全链路透传） |
...
- US-DA-4：作为数据分析师，我希望多指标/多维度对比分析并可保存分享，以便快速定位差异与趋势（对应 R3-12 多指标对比，5.4.3a）。[R4-09]
- US-DA-5：作为数据分析师，我希望按指标集快速检索一组相关指标，以便一次性消费业务场景所需指标组合（对应 R3-15 指标集，4.5 指标集段）。
...
- US-MO-4：作为指标 Owner，我希望编写指标消费指南（适用/不适用/常见误用），以便消费方正确使用指标而非误读口径（对应 R3-20 消费指南，5.4.3 消费指南 Tab）。
...
- **Schema Drift 检测与影响分析 [R3-04]**：生产中源表结构变更（增删列、改类型、删表、重命名）频繁且常无通知，须自动检测并联动指标体系——① **检测机制**：增量采集时比对前后 `db_catalog` 快照，检测 `CREATE_TIME`/`ALTER_TIME` 变化（关系库读 `information_schema.TABLES`）或 DDL 事件；检测频率与增量采集一致（定时 + 手动触发）；② **变更分类与兼容性评估**：新增列→兼容（无影响，自动入库）；列改名→兼容但须更新血缘映射；列删/类型变更→破坏性（INT→BIGINT 无影响，DECIMAL→VARCHAR 有影响，类型变窄如 VARCHAR(255)→VARCHAR(50) 有影响），自动标记 `breaking_change=true`；删表→严重；③ **影响分析自动化**：检测到破坏性变更后，沿 4.4 血缘遍历引用该列的指标，生成"Schema 变更待确认"待办（列出受影响指标 + 变更类型 + 建议处置——删列→指标引用断裂需改口径/废弃；类型变更→重新试算验证）；④ **删表处理**：源表 DROP 后，引用该表的指标自动标 `DATA_SOURCE_DROPPED` 并生成 Owner 待办，Owner 须在 7 天内决定（改来源表/标 DEPRECATED/确认误报）；⑤ **表重命名**：检测到 DROP+CREATE 同结构不同名时，自动建议"重命名映射"并更新血缘，避免重复注册；⑥ 检测事件入 4.10 审计与治理驾驶舱，按严重级通知（删表 P0、列删/类型变 P1、新增/列改名 P3）。
- **源表删除/废弃处理 [R3-10]** [R4-01]：生产中源表被 DROP 或整库下线是高影响事件，须闭环处理——① **源表 DROP**：与 R3-04④ 删表处理为同一场景，此处补充逾期升级机制——逾期未处理自动升级 `domain_admin` 并在驾驶舱标红（R3-04④ 已定义 7 天待办，此处为补充升级逻辑）；② **表重命名**：与 R3-04⑤ 表重命名策略为同一场景，此处补充结构相似度阈值（一致率 > 90%）与一键确认机制；③ **数据库/Schema 级批量变更**：整库下线或迁移时，支持 `domain_admin` 批量处理：按库筛选所有受影响指标→批量改绑/批量 DEPRECATED，走 4.9.7 批量权限操作策略（含影响预览+逐条审计）。④ 所有变更事件入 4.10 审计，驾驶舱展示"源端变更影响"专项看板。
- **与 PIPL 对齐**：PII 字段访问/推导单独留痕（见 4.10 审计）。
- **资产认领与归属**：采集到的表/字段若**无 Owner**（孤儿资产），标记为"待认领"并在治理驾驶舱展示；定期推送认领提醒，超期未认领可自动回收或指派域管理员代管。人员离职时触发资产交接流程，确保资产不悬空（与 4.5 指标 Owner 机制对齐）。[R3-14] **认领激励与责任闭环**——① **认领激励**：认领孤儿指标/表计入"治理贡献"积分（治理驾驶舱可见），月度治理贡献 TOP 榜公示；积分与治理 OKR 联动（5.5.6），而非直接与绩效挂钩（避免为积分而认领）；② **认领后责任边界**：认领后 30 天内须完成口径完善（定义/维度/来源/计算公式齐全率 ≥ 80%，呼应 5.5.3 口径完整度维度），未完成标"认领未完善"在驾驶舱展示；③ **拒绝认领的处理**：连续 60 天无人认领的孤儿指标，由 `domain_admin` 评估——有消费则指派代管人；无消费则进退役建议流程（4.5 指标退役标准流程）；④ 认领/代管/退役事件入 4.10 审计。
...
6. **去重与冲突预检**：候选生成后与已注册指标做名称/口径相似度比对（联动 4.7 冲突检测）——已存在则提示"合并/复用"而非新建；同名不同义提前预警，降低后续仲裁成本。[R11-03] **与 4.7.2 权重与阈值对齐**：去重与冲突预检复用 4.7.2 相同的综合分计算（名称 0.4 + 口径 0.4 + 血缘 0.2）与硬冲突/软冲突阈值（综合分 < 0.6 判硬冲突、∈ [0.6, 0.85) 判软冲突），保证 4.3 预检与 4.7 正式检测结论一致，避免"预检无冲突但后续检出"的矛盾。
...
- RDBMS 持久化：`llm_model_config` / `llm_test_report` / `prompt_template` / `prompt_template_version` / `golden_set` / `calibration_result`。[R3-25] **图模型用途说明**：LLM 配置模块的 Neo4j 节点/边（4.3a.7 定义）仅用于运行时健康看板的可视化查询（如"某模型服务了哪些能力→哪些 Prompt→最近校准结果"），**不与 RDBMS 双写**——LLM 配置的权威数据源为 RDBMS（事务一致性保证），Neo4j 为只读同步视图（由图库同步器从 RDBMS ETL 导入，频率 5min）。6.3 ER 图仅包含 RDBMS 实体，Neo4j 节点/边不纳入，避免双写复杂度与一致性风险。
...
2. **调度 DAG 解析**：从调度依赖（Airflow/自研平台）提取任务→任务、任务→产出表，构建表级（L1）上游链。[R3-05] **调度依赖与数据血缘的双向校验**：调度 DAG（任务→任务依赖）与数据血缘（表→表/字段→字段依赖）是两种正交但应一致的依赖视图，须双向校验——① **有调度依赖但无血缘**：调度显示 A→B 但血缘未检出 A 产出表→B 来源表的边，可能为"弱依赖"（如 A 仅触发 B，不供数据）或血缘解析遗漏，标记 `schedule_without_lineage` 供人工确认；② **有血缘但无调度依赖**：血缘检出表 X→Y 但调度未见对应任务依赖，可能遗漏调度配置（X 产出后 Y 不自动触发，需人工跑），生成"疑似遗漏调度配置"告警通知 `domain_admin`；③ **校验频率**：增量血缘构建后自动触发校验，差异结果入治理驾驶舱；④ 校验不阻断血缘或调度主流程，仅提示与记录。
...
- **血缘版本/时间旅行**：口径版本切换时血缘按版本快照可回溯，能查"某指标 v2 对应哪条上游链路"。[R10-04] **血缘边变更快照**：不仅口径版本切换触发快照，血缘边本身变更（源表改名→边重定向、字段删除→边断链、Parser 重新解析→边更新）亦触发旧边快照——保留 `before_edge`（旧源/旧目标/旧变换表达式/旧 confidence）+ `after_edge` + `变更原因`+ `触发事件`（schema_drift/reparse/manual），供事后回溯"某指标在 2025-03-01 前的上游链路是什么"。快照入 `lineage_edge_history` 表，保留期与审计一致（≥180 天热存）。
...
- **跨系统断层**：源库→数仓→BI 之间若存在手工导出/文件摆渡/Excel 中转，该段血缘天然断链，需人工登记"外部依赖"边。[R10-03] **断链前端可视化**：血缘图中断链处显示虚线框节点标"外部依赖（未接入）"+ 一键"登记外部依赖"入口（填写外部系统名/数据流向/负责人/更新频率）；人工登记的外部依赖边标 `method=manual` + `confidence=高`（人工确认优先于 LLM 推断，与 Parser 解析边同权重），与 LLM 推断边（`method=llm`/`confidence=中`）在视觉上以实线 vs 虚线区分。
...
- **维度变更影响分析与变更评审联动 [R3-02]**：维度变更分两类，差异化处理——① **维度结构变更**（层级链增删、上卷规则修改、SCD 策略切换）属破坏性变更，须走 4.5 变更评审：提交时自动跑影响面（4.4 what-if，列出受影响指标及其下钻路径），`domain_admin` 审批后方可生效；生效后关联指标的维度版本快照同步更新。② **维度值变更**（枚举值增删改、合并拆分、映射规则变更）影响范围较结构变更窄，但核心维度（地区/渠道/品类）值变更仍须评估——新增值自动入库无需审批；删除/合并值须检查被删值是否仍被指标引用（查询日志 4.11 近 30 天有消费），有引用则生成"维度值退役待办"（类似指标退役流程：30 天通知期→引用清空→删除）；合并/拆分须更新映射规则并通知关联指标 Owner。变更事件入 4.10 审计与治理驾驶舱。
...
**指标间业务关系建模 [R3-13]**：4.4 血缘是物理关系（数据从哪来），4.5 指标树是分类关系（属于哪个主题），但业务侧还需**语义关系**——① **先行指标（Leading Indicator）**：A 先于 B 发生变化（如 DAU 先行于 ARPU），声明 `LEADS`（A→B，带滞后周期如"通常领先 1-2 周"），用于 4.17 推荐找数（查 B 时推荐"先行指标 A"）与驾驶舱趋势预判；② **因果关系（Causal）**：A 变化导致 B 变化（如投放支出→获客量），声明 `CAUSES`（A→B，带因果强度 high/medium/low），用于 4.7 冲突仲裁参考（因果链上的指标优先为"权威源"）与新员工理解业务流程；③ **同属业务流程（Process）**：多个指标属于同一业务流程的不同环节（如订单量→支付量→发货量→退货量），声明 `BELONGS_TO_PROCESS`（指标→流程名+步骤序号），用于指标目录"流程视图"（按流程展示指标链）与 4.17 推荐"同流程相关指标"。④ **建模方式**：关系声明在指标详情页"业务关系"Tab（5.4.3 补充），由 `domain_admin`/`metric_owner` 维护，LLM 可基于 ETL 链路/业务术语自动建议候选关系（标记"推断"待人确认）；⑤ **存储**：Neo4j 新增边类型 `LEADS`/`CAUSES`/`BELONGS_TO_PROCESS`（带属性：lag/causal_strength/process_name/step_order），RDBMS 新增 `metric_business_relation` 表。⑥ **一期范围**：仅支持人工声明 + LLM 候选建议，自动推断为二期。[R4-05] **业务关系边与血缘边的语义区分**：`LEADS`/`CAUSES` 是"人声明的业务语义关系"（非自动推导，可能有误须人确认），而 `DERIVED_FROM`（4.4 血缘）是"解析出的物理/计算依赖"（SQL 静态分析或运行时日志提取，客观事实）。两者正交可并存——例如指标 A 因果于指标 B（`CAUSES`）但 A 并非从 B 的 SQL 计算得出（无 `DERIVED_FROM` 边）；反之 A 派生自 B（`DERIVED_FROM`）但无因果关系。UI 须在血缘图与业务关系图上区分展示（血缘用实线，业务关系用虚线+标签）。

**废弃与替代（successor）**：`DEPRECATED` 时须指定 `successor`（替代指标）；消费方（API/QuickBI）请求已废弃指标时自动返回替代指标或明确迁移指引，避免消费端静默报错，配套 F11 级联校验。

**指标归属域变更 [R3-18]**：生产中指标归属域可能变更（如"客单价"从运营域迁移到交易域），须闭环处理——① **变更审批**：归属域变更由 `metric_owner` 发起，须经**原域** `domain_admin`（释放）+ **目标域** `domain_admin`（接收）双方审批；跨域变更自动触发 4.7 冲突预检（目标域是否已存在同名/同义指标）；② **权限重映射**：变更后指标权限从原域迁移到目标域——原域授权自动回收，目标域按默认策略授权（目标域 `domain_admin` 可调整）；已授权消费方经 4.14 通知"该指标已迁移至 XX 域，请确认权限"；③ **健康度影响**：指标从原域健康度统计移出，计入目标域；原域健康度评分重算（移除该指标贡献），目标域同理；④ 血缘/引用不受影响（指标 `metric_code` 不变，仅 `domain` 字段变更），但血缘图中域标签更新。⑤ 变更事件入 4.10 审计与治理驾驶舱。

**指标退役标准流程**：从 DEPRECATED 到彻底删除须经过以下步骤：
1. **退役判定**：满足以下条件之一时可建议退役——① 连续 90 天无消费（4.11 查询日志为零）[R11-01] **"消费"口径声明**：4.11 查询日志须覆盖三条管线——Semantic API 调用 + QuickBI 嵌入查询 + MCP 工具调用（4.12），统一入 `operation_audit`（4.10）。仅当三条管线均无记录时方可判定"无消费"，否则 QuickBI 高频消费但 API 无调用的指标可能被误判退役；② 已有 `successor` 且 successor 消费量稳定 > 30 天；③ `metric_tier = Tier-3` 且 Owner 主动申请退役。
...
4. **回收站（软删除）**：校验通过后指标进入回收站，保留 30 天可恢复。[R6-06] 此期间指标仍可通过状态机 `DEPRECATED → DRAFT` 复活路径恢复（须重新走审核），回收站内指标详情页显示"回收站中，距彻底删除还剩 N 天"提示。
...
**生命周期状态机** [R6-01]：
```
DRAFT → PENDING_REVIEW → PUBLISHED ⇄ DATA_SOURCE_DROPPED
                           ↓    ↑              │
                      EXPERIMENTAL              │
                           │                    ↓
                           └── 灰度全量 ──→ DEPRECATED ←──┘
                                           ...
                                           DRAFT (复活，须重新审核)
```
- **PUBLISHED → EXPERIMENTAL**：审核通过选灰度，仅白名单可见（5.4.6）。
- **PUBLISHED → DATA_SOURCE_DROPPED**：源表 DROP/不可达自动标记（4.2 R3-04④/R3-10①），7 天待办，恢复→回 PUBLISHED，不可恢复→转 DEPRECATED。
- **EXPERIMENTAL → PUBLISHED**：灰度观察期满一键全量。
- **EXPERIMENTAL → PENDING_REVIEW**：灰度回滚。
- **DEPRECATED → DRAFT**：复活须重新走审核（仅在回收站保留期内可复活）。
- **F2 门禁**：无 Owner 不可 PUBLISHED。
- **F11 级联校验**：DEPRECATED 前须通过下游引用校验，避免悬挂依赖。
- **版本化**：指标进入 `PENDING_VERSION` 子状态，消费方确认后升为当前生效版本；破坏性变更须显式标注。[R3-01] **PENDING_VERSION 期间消费侧行为规范**：① 旧版本查询 meta 须标注 `pending_upgrade=true`（含待生效版本号与预计切换时间），提示消费方"新版本即将生效"；② 版本切换瞬间的在途查询策略——以 API 收到请求时的 `metric_version` 为准，切换后新请求自动取新版本，在途请求不被中断；③ 消费方可"钉住"特定版本（请求带 `metric_version=<N>`），钉住版本在 Sunset 期（默认 30 天）后返回 `410 VERSION_SUNSET` 并引导升级，避免永久钉住旧口径；④ 钉住版本进入 Sunset 前 7 天经 4.14 通知消费方。[R4-04] **注意区分"口径版本钉住"与"API 协议版本废弃"**：本条 Sunset 机制针对指标口径版本（同一指标的新旧计算逻辑切换），与 4.11.12 Semantic API 端点自身的协议版本废弃（如 v1→v2 API 路径升级，6 个月 Sunset 周期）是完全独立的两套机制——前者由指标 Owner 管控（业务口径演进），后者由平台运维管控（API 兼容性），两者互不影响。[R9-02] **PENDING_VERSION 期间 Schema Drift 处理**：版本待生效期间若 4.2 Schema Drift 检测到源表结构变更（如字段删除/类型变更），新版本口径可能已失效——自动暂停版本切换并通知 Owner："源表结构已变更，请复核新版本口径是否仍有效"；Owner 须在 7 天内确认或重走口径修改→重新提交审核；逾期未处理则自动回退到旧版本（PENDING_VERSION→取消），避免失效口径静默生效。
...
- **口径一致性对账（同源校验）**：同一业务事实被多个指标引用时，须支持"同源指标对账"——如 `GMV` 与 `销售额` 若源自同一事实表不同口径，定期跑一致性校验，差异超阈值告警，防"同表出俩数"。这是跨域协作（FR-09）可信的前提。[R3-06] **口径一致性对账调度策略**——① **对账频率**：Tier-1 指标每日对账（随分区产出就绪自动触发）；Tier-2 每周对账；Tier-3 不自动对账（按需手动）；② **对账范围**：同源（同一 `data_set`）指标组为最小对账单元，跨源对账为二期；③ **差异处理流程**：差异超阈值→生成 `QualityIncident`（4.8）→ Owner 待办确认（合理差异如口径边界不同则关闭并留因；口径有误则走 4.5 变更评审）→ 结论入 4.10 审计；④ **资源约束**：对账查询走 4.11 `BATCH` 优先级（4.11.9），受 `data_source.quota` 约束，超限排队不抢占交互式查询；⑤ 对账结果入治理驾驶舱"同源对账合格率"。
...
- **指标结果快照存证**：合规/争议仲裁常需证明"某天某报表上这个数是多少"（as-of 历史值），仅靠口径版本（`metric_version`）不够。对 Tier-1 及涉财务/合规指标，落**结果快照**（`metric_value_snapshot`：`metric_code` + 口径版本 + 维度组合 + 时间窗 + 值 + 质量分 + 生成时间 + 生成方式），随 4.11 即时查询或 6.1 物化落库，**只写不删（WORM）**；保留期按合规要求（默认 ≥ 180 天热存，之后冷归档，呼应 4.10 审计归档）。用途：① 4.7 争议仲裁的证据回溯；② 外部审计/监管报送的数值佐证；③ 跨期对账（"上月报给财务的数 vs 本月口径"）。快照导出受 4.9 权限 + 行数上限约束，PII 维度值脱敏。[R3-09] **快照生命周期补充**——① **触发时机**：Tier-1 指标分区产出就绪 + 4.8 质量校验通过后自动触发快照落库（校验未通过仍落库但标 `quality_downgraded`，保留"当时实际值"）；非 Tier-1 但涉财务/合规指标由 Owner 手动开启快照。② **快照粒度**：默认全维度组合落库；维度基数过大（如 UV 按 UID 维度）时仅存核心维度组合（Owner 配置，默认为指标默认下钻维度集 + 时间维度）。③ **存储策略**：热存 180 天（MySQL 分区表按月分区），之后自动冷归档至对象存储，归档后查询走异步导出（T+1）；冷归档策略与 4.10 审计归档一致。④ **查询接口**：`GET /api/v1/metrics/{id}/snapshots?dateRange=&dimensions=`，返回历史快照值，受 4.9 权限约束；消费方可用此接口做跨期对账。[R4-03] **快照接口与 /query 的关系决策**：快照接口为独立端点而非复用 4.11 `/query` + `as_of_date` 参数——理由：快照为 WORM 存证（只写不删），/query 为即时计算引擎，两者存储路径与权限模型不同（快照受 WORM 保留期约束，/query 受缓存 TTL 约束）；若消费方需"某版本口径下的历史值"而非"存证值"，应走 /query + `metric_version` 参数而非快照接口。⑤ **与口径版本关联**：快照含 `metric_version`，口径版本切换后新旧快照均保留，可按版本号查询"某版本口径下的历史值"。
...
**"数据产品"概念预留（二期/三期方向）[R3-19]**：业界 Data Mesh 趋势下指标以"数据产品"打包交付（=指标集 + 维度集 + SLA + Owner + 文档 + API 契约），当前平台只有单指标消费。一期不实现但预留设计空间——① 指标集（R3-15）是数据产品的雏形，后续可扩展为"数据产品目录"（含 SLA 承诺、产品版本、订阅管理）；② 4.11 Semantic API 的指标集查询接口（`/metric-sets`）可平滑升级为产品级 API 契约；③ 5.3 页面清单预留"数据产品目录"导航位（一期灰显）。
...
**治理驾驶舱**：各域健康度评分、冲突趋势、低活跃指标清理，供治理委员会决策。健康度评分由可度量维度加权构成：① 新鲜度达标率（freshness SLA）；② 质量异常率（4.8）；③ 跨域同名不同义冲突数（4.7）；④ 使用率/活跃度；⑤ 文档完整度（单位/口径/维度/Owner 是否齐全）。评分维度与权重在运营看板透明可见，不可度量则不可治理。[R3-29] **域间对比与标杆**——① **域间对比视图**：驾驶舱新增"域间对比"Tab，横向展示各域健康度评分/冲突数/覆盖率的对比柱状图，支持按时间维度展示纵向趋势（月度变化折线）；② **标杆机制**：健康度评分最高的域标记为"标杆域"，其他域可展开标杆域的评分维度明细做对标（"交易域健康度 85，你的域 72，差距在'文档完整度'维度"）；③ 对比结果可导出供治理委员会月度复盘。
...
**指标消费指南/使用手册 [R3-20]**：4.5 有口径定义、4.6 有术语库，但缺"如何正确使用这个指标"的消费侧文档——① **定义**：指标详情页新增"消费指南"Tab（或折叠区），由 `metric_owner`/`domain_admin` 编写，包含——**适用场景**（"GMV 适用于衡量交易规模，不适用于衡量利润"）、**不适用场景**（"GMV 不可跨渠道简单相加，因存在跨渠道重复下单"）、**常见误用**（"UV 不可跨日相加，须走重算"）、**与其他指标的关系/对比**（"GMV vs 净收入：GMV 含退款，净收入不含"）、**推荐维度组合与图表类型**；② **与口径定义的区别**：口径定义是"怎么算的"（技术逻辑，客观描述），消费指南是"怎么用的"（业务语义，主观建议），两者正交——口径卡由系统/LLM 自动生成（公式+维度+来源），消费指南由 Owner 编写（适用/不适用/误用场景），后者非自动生成且可能存在主观偏差，消费方须结合口径自行判断；③ **LLM 辅助**：4.3 LLM 可基于口径+术语+血缘自动生成消费指南草稿（标记"AI 生成待人确认"），Owner 审核后发布；④ **与 4.6 术语库联动**：消费指南中的业务概念自动链接到术语库概念卡；⑤ 消费指南随指标版本化，口径变更后 Owner 须更新消费指南（变更评审提示）。

**指标集/指标组（消费侧概念）[R3-15]**：分析师以"一组指标"为消费单位（如"交易核心指标集" = GMV + 订单量 + 客单价 + 支付转化率），当前只有指标树（治理视角分类）和模板包（生产视角克隆），缺少消费侧的指标集——① **定义**：指标集 = `{名称, 描述, 指标列表[N], 默认维度集, 默认时间范围, Owner, 可见范围(域/公开)}`；② **管理**：指标集由 `metric_owner`/`domain_admin` 创建与维护，支持增删指标、调整默认维度与时间范围；指标集独立版本化（指标增删产生新版本，消费方绑版本）；③ **消费交互**：指标目录/首页提供"指标集"入口，点击进入指标集详情→一键出表/出图（4.11 `/query` 批量查询）→可保存为 QuickBI 看板或生成 API 查询模板；④ **API 支持**：`GET /api/v1/metric-sets`（检索）/ `GET /api/v1/metric-sets/{id}`（详情）/ `POST /api/v1/query/batch`（按指标集批量查询），复用 4.11 鉴权/限流/降级底座；⑤ **推荐联动**：4.17 找数推荐在指标详情页推荐"包含此指标的指标集"；⑥ **与模板包区分**：模板包是生产侧克隆（注册新指标），指标集是消费侧组合（出图/出表/出 API），两者正交。
...
- **术语关联可视化 [R3-28]**：概念卡扩展为"术语关系图"——① **术语→指标关联图**：以术语为中心节点，放射状展示引用该术语的所有指标（按域着色，标注指标状态），一目了然"这个概念被谁引用、分布在哪些域"；② **术语间关系**：新增术语间关系类型——`SYNONYM_OF`（同义，如"GMV"同义"流水"）、`BROADER_THAN`（上位，如"收入"上位于"GMV"）、`NARROWER_THAN`（下位，如"付费 GMV"下位于"GMV"）、`RELATED_TO`（关联，如"订单量"关联"GMV"），由 `domain_admin`/`metric_owner` 维护，LLM 可自动建议候选关系（标记"推断"待人确认）；③ 术语关系图在术语库详情页渲染，支持点击跳转关联术语/指标。
...
3. **及时性（Timeliness / SLA）**：产出时间相对预期 SLA 的延迟（如 `T+1` 应在 06:00 前就绪），未达标标记"数据延迟"（见 4.2 采集 SLA，时区须与产出任务一致，防 `T+1` 边界错位）。[R3-03] **延迟通知与容忍度的差异化策略**——按 `dw_layer` × `metric_tier` 矩阵差异化处理：① DWS/ADS 层延迟不可接受（直接影响下游报表与 API 消费），须立即告警 Owner + 标注消费侧 `freshness=delayed`；ODS/DWD 层延迟可宽限（如宽限至 08:00，因明细层通常非直接消费端），仅标记不告警。② Tier-1 延迟立即 P0 告警（通知 Owner + 所有关注者 + 治理委员会），Tier-2 延迟 P1 告警（Owner + 关注者），Tier-3 仅标记 `delayed` 不主动告警。③ 延迟恢复后须发送"数据已补齐"通知（含补齐分区范围 + 补齐时间 + 质量复检结果），而非简单"已恢复"，让消费方确认数据可信后再决策。差异化策略配置在 4.5 SLA 契约中声明，治理驾驶舱按矩阵聚合展示。
...
- **告警疲劳风险**：规则过密会淹没关键告警，需严重级分流 + 去重聚合（同根因合并一条），运营反馈（4.10）调阈值。[R4-10] **与 R3-03 延迟差异化策略的对齐**：R3-03 按 `dw_layer` × `metric_tier` 精细化了告警分级（更多级别、更多通知对象），虽提升了告警精准度但也增加了告警总量——须确保 4.14 同根因聚合机制（4.14.2）仍生效：同一上游延迟导致的多个 Tier-1/2 告警须合并为一条摘要（而非按 Tier 分别发送），避免精细化反而加剧告警疲劳。
...
- `compliance_officer`：数据合规复核（含 PII/CONFIDENTIAL 来源指标的发布合规门禁）。[R7-02] **域范围**：`compliance_officer` 为跨域角色（全公司统一，不绑定单个主题域）——PII 合规是公司级责任，不应因域边界导致合规盲区；PDP 判定时 `compliance_officer` 可复核任何域的 PII 指标，无需额外 `domain_grant`
...
#### 4.9.7 批量权限操作策略 [R3-07]
- **批量授权/回收**：`domain_admin`/`platform_admin` 可批量授权或回收多个用户×多个指标/维度的权限，适用场景：新项目组成立（批量授权一组指标给新团队）、角色变更（某人转岗需批量回收旧域权限+批量授权新域权限）。
- **影响预览（dry-run）**：批量操作前须展示影响预览——受影响用户数、受影响指标/维度数、权限变更类型（新增/回收/降级），管理员确认后方可执行；预览结果可导出供审计。
- **审计格式**：批量操作审计逐条记录（每条含 actor/target_user/resource/action/before/after），同时在批量维度生成一条汇总记录（batch_id + 操作总数 + 成功/失败计数），兼顾细粒度溯源与批量审查效率。
- **失败回滚**：批量操作按用户粒度独立事务——单用户失败不影响其他用户；部分失败时报告失败清单并建议重试，不自动回滚已成功项（已成功项可一键撤销）。批量操作进度在待办中心可见。

#### 4.9.8 图模型与存储（Neo4j + RDBMS）
...
- **角色枚举固定 [R9-09]**：一期固定 6 角色枚举（4.9.2），不支持自定义角色；大型组织的"域只读审计员""外部合规顾问"等角色需求在二期评估——若开放自定义角色，权限须为 6 基础角色权限子集的组合，RBAC×域权限矩阵须同步扩展。

#### 4.9.10 生产场景补充（MVP 后可逐步启用）
...
#### 4.10.1 审计日志模型与采集管线
- **事件类型枚举**：注册/提交/审核(通过/驳回)/发布/版本切换/废弃/替换、冲突裁决、质量告警确认、权限申请/授权/回收、**数据分级变更（含 PII 降级/升级，R7-01）**、**平台配置变更（含限流/配额/阈值/SLA 调整，非仅 LLM，R9-07）**、查询(页面/API)、LLM 解析、导出/订阅、登录/越权尝试。
...
8. **成本归属与核算**：
   - **成本维度**：LLM 调用（按 token 单价，4.3a）、查询下推（按扫描行数/计算时长）、物化存储（按临时表容量）、ES 存储（按索引大小）。
    - **归属规则**：按域 × 消费方聚合——每域的 LLM 解析/查询/物化成本计入域级成本中心；Semantic API 按客户端 `api_client_id` 归属消费方。[R10-01] **共享成本与免费额度**：① **共享成本不分摊**：一期 LLM 解析成本归属指标所在域（谁注册谁承担），查询成本归属消费方（谁查谁承担），两者独立归因不做交叉分摊——避免分摊逻辑复杂度阻碍一期交付；二期视需求评估"跨域消费成本分摊"（如 A 域指标被 B 域高频查询，B 域是否分摊 A 域解析成本）。② **免费额度**：每 `api_client` 每月前 10,000 次 API 调用免费（含查询/检索），超出部分按量归因；每域每月前 1,000 次 LLM 解析免费，超出按 token 计费。免费额度在 API 控制台与成本看板可见，目的为降低采纳门槛。③ **结算性质**：一期成本为虚拟归因（度量看板展示趋势与占比，不实际结算），二期视公司内部结算制度决定是否接入实际计费系统。
...
#### 4.10.2a 平台健康仪表盘 [R3-16]
4.10 运营度量面向业务，4.13 降级面向故障处理，但缺**平台自身健康的统一视图**——将以下聚合指标纳入 `platform_admin` 专属仪表盘：
| 健康维度 | 指标 | 数据源 | 刷新频率 | 告警阈值 |
|---------|------|--------|---------|---------|
| API 健康 | 成功率 / P99 延迟 / 降级率 | 4.11 API 日志 | 1min | 成功率 < 99% / P99 > 1s / 降级率 > 5% |
| 采集覆盖 | 已采集表/字段占全量比例 | 4.2 db_catalog | 1h | 核心表覆盖率 < 95% |
| 血缘解析 | 字段级血缘覆盖率 / 解析失败率 | 4.4 血缘图 | 1h | 覆盖率 < 80% / 失败率 > 10% |
| LLM 可用 | 4.3a 健康看板 6 指标 | 4.3a | 30s | 可用性 < 95% / P95 > 15s |
| 数据源连通 | 各 DataSource 连通状态 | 4.2 探测 | 5min | 任一核心源不可达 |
| 存储容量 | MySQL/Neo4j/Redis/ES 容量利用率 | 基础监控 | 5min | 任一 > 80% |
仪表盘 SLA：刷新频率 ≤ 上表定义；`platform_admin` 与运维 SRE 可见；健康维度综合评分 < 70 时自动触发 4.13 降级评估（非自动降级，由 `platform_admin` 确认）。[R6-03] **与模块级健康看板的关系**：本仪表盘是全局汇总视图，各模块有专属详情看板——LLM 模块详情见 4.3a.3 健康看板（可用性/延迟/错误率/Token/队列/降级事件），数据源详情见 4.2 采集状态页，血缘详情见 4.4 解析监控。点击仪表盘任一维度可下钻跳转到对应模块详情页（如点击"LLM 可用"→跳转 4.3a.3 LLM 健康看板），形成"全局概览→模块下钻"的监控层级。
...
- **消费方满意度度量闭环 [R3-11]**：① **API 消费方满意度**：内部用户用问卷，但 API 消费方（DataAgent/外部系统）不适用问卷——改用代理指标度量：API 调用成功率 / 降级率 / 429 触发频率 / retry 率 / `trace_id` 级问题反馈率，低满意度的消费方由 `platform_admin` 定期回访；② **数据可信度主观度量**：分析师"敢不敢用平台指标做决策"无法从客观度量推断——在关键场景（如首次使用某指标出报表/完成一次溯源）插入轻量"这个指标可信吗？（1-5 星）"评价，评价结果纳入指标健康度参考维度（4.5.3 补充"消费可信度"子维度，权重 5%）；③ **满意度与北极星关联验证**：若满意度高但 1.4 耗时/冲突指标不改善，视为"虚假满意"（习惯性好评），须回检样本偏差（是否仅回访了活跃用户而忽略了沉默流失）。
...
**通用请求/响应规范 [R13-12]**：
- 请求必带 Header：`Authorization: Bearer <token>` + `Content-Type: application/json`
- 响应必含 Header：`X-Request-ID`（请求唯一标识）+ `X-Trace-ID`（4.10 审计关联追踪 ID）
- API 版本响应 Header：`X-API-Version: v1`
- 可选请求 Header：`Accept-Language: zh-CN`（国际化预留）
- 所有错误响应体统一格式：`{"error_code": "<CODE>", "message": "<人类可读描述>", "retry_after": <秒|null>, "trace_id": "<追踪ID>"}`（附录 A.1）

- 端点集合：
  - `GET /api/v1/metrics`：检索指标。[R9-04] **分页规范**：默认 `pageSize=20`，最大 `pageSize=100`（超限返回 `422`），响应结构 `{"total": <总数>, "page": <当前页>, "page_size": <页大小>, "data": [...]}`；超范围请求（page 超出总页数）返回空 `data`（非 404）；分页参数与附录 B `search.page_size_default`/`search.page_size_max` 联动。**[R13-01] 排序参数**：`sort=created_at|updated_at|quality_score|popularity, asc|desc`，默认 `updated_at desc`；排序字段须与 ES 索引字段对齐。**[R13-02] 过滤参数**：除 `q`/`domain`/`tag`/`page` 外，补充——`status`（多选，逗号分隔，默认仅 `PUBLISHED`）、`tier`（1/2/3）、`dw_layer`（ODS/DWD/DWS/ADS/DM）、`owner`（用户 ID）、`has_pii`（boolean），均可选。**[R13-15] `q` 参数语义**：全文搜索（匹配名称/口径描述/注释/同义词/术语），ES 索引驱动；支持精确匹配（引号包裹 `"gmv"`）与通配符（`*`），不支持布尔表达式；搜索排序由 ES relevance score + 指标热度加权。
  - `GET /api/v1/metrics/{id}`：口径详情。**[R13-03] 响应 Schema**：`{id, metric_code, name, status, version, type(atomic|derived|composite), granularity, unit, aggregation, time_semantics, freshness, serving_mode, dimensions[{name, is_required, granularity_level}], expression, quality_score, quality_flag, lineage_url, tier, dw_layer, sensitivity_level, created_at, updated_at, owner}`
  - `POST /api/v1/metrics/batch`：批量指标详情 [R13-13]。入参 `{metric_codes: string[]}`（上限 20），返回每个指标详情的映射；权限不足/不存在/已废弃按 batch 规范单独返回错误码（复用 `BATCH_PARTIAL_FAILURE` 207）。一期可选（消费方可并行调 GET），二期补齐。
  - `GET /api/v1/metric-sets`：检索指标集（R3-15，支持 `q`/`domain`/`page`）。[R4-07] **[R13-04]** 指标集检索复用同一分页规范（默认 20，最大 100，响应结构同 R9-04）。
  - `GET /api/v1/metric-sets/{id}`：指标集详情（含指标列表+共享维度+推荐联动）。
  - `GET /api/v1/metrics/{id}/snapshots`：指标结果快照查询（R3-09，支持 `dateRange`/`dimensions`）。**[R13-05]** 补分页参数 `page`/`pageSize`（复用 R9-04 分页规范），快照结果按维度组合 × 时间分页。
  - `POST /api/v1/query`：即时计算，下推到绑定 DataSource。
  - `POST /api/v1/query/dry-run`：试算沙箱（见 4.11.6）。
  - `GET /api/v1/lineage/{metric_code}`：消费侧血缘查询入口（复用 4.4）。**[R13-10]** 补参数：`depth`（1=L1 表级，2=L2 字段级，默认 1）、`direction`（upstream|downstream|both，默认 upstream）、`confidence_min`（过滤低置信度边，默认 0.6）；响应结构：`{nodes[{id, type, name, properties}], edges[{source, target, type, confidence, method, transform_expr}], meta: {direction, depth, total_nodes, total_edges}}`
- 仅返回结构化指标定义与计算结果，**不含生成式 SQL**（NL2SQL 属 4.12）。
- `POST /query` 入参规范：`metrics`（指标码数组）、`dimensions`（须为该指标合法关联维度）、`filters`（维度值过滤）、`dateRange`（时间范围 + 粒度）、`comparison`（同比/环比）、`orderBy`、`page`/`pageSize`。**[R13-06] 幂等性声明**：`POST /query` 为非幂等（每次可能触发实时下推），消费方重试须带 `idempotency_key`（UUID），平台 24h 内相同 key 返回缓存结果不重复下推；`dry-run` 同理。**[R13-07] `comparison` 结构**：`{type: "yoy"|"mom"|"wow", offset: 1|2（往前 N 期）, base_date: "2025-01-01"（可选自定义基期）}`；返回结构补 `comparison_data` 字段含基期值与变化率。**[R13-08] `orderBy` 结构**：`{field: "<metric_code>"|"<dimension_name>", direction: "asc"|"desc"}`，默认空（不排序）；按指标值排序时指标须为 ADDITIVE（4.5 additivity），NON_ADDITIVE 排序返回 `ADDITIVITY_VIOLATION`。
- `POST /query/batch` 入参规范 [R6-02]：`metric_set_id`（指标集 ID，与 `metrics` 二选一）、`metrics`（指标码数组，与 `metric_set_id` 二选一）、`dimensions`（共同维度集，须为所有涉及指标的合法交集）、`filters`、`dateRange`、`orderBy`、`page`/`pageSize`。约束：① `metric_set_id` 模式自动展开为指标集内所有指标，维度取指标集默认维度集；② 批量查询上限 20 个指标（呼应 R3-17 批量注册上限），超限返回 `422 BATCH_QUOTA_EXCEEDED`；③ 任一指标权限不足/不存在/已废弃，该指标单独返回错误码，其余指标正常返回（`207 BATCH_PARTIAL_FAILURE`）；④ 批量查询走 `BATCH` 优先级（4.11.9），受 `data_source.quota` 约束。**[R13-09] 维度交集计算**：平台自动求交集——消费方传入的 `dimensions` 为"期望维度"，平台校验后取所有涉及指标的合法维度交集，非交集维度返回 `DIMENSION_INVALID`；消费方也可不传 `dimensions`，平台自动使用交集。
- 返回结构：`data`（维度组合 × 指标值）+ `meta`（**标准语义标注**，见 4.11.7）。越界请求（细于粒度、非法维度）按 4.5 下钻硬约束返回降级结果或 `422`；不支持操作显式报错，不静默返回错数。
- **写操作 API 能力边界 [R9-03]**：本节端点为**只读查询**（GET + POST /query 系列），指标注册/提交审核/审核通过/驳回/废弃/复活等写操作一期仅走前端 UI；二期逐步开放写 API（`POST /api/v1/metrics` 注册、`PATCH /api/v1/metrics/{id}/status` 状态迁移、`DELETE /api/v1/metrics/{id}` 废弃），写 API 须复用 4.9 PDP 权限 + 5.5.1 乐观锁 + 4.10 审计；四期 DataAgent MCP（4.12）提供完整写能力工具集。[R11-08] **指标集 CRUD 归属**：指标集创建/编辑/删除（`POST /api/v1/metric-sets`、`PUT /api/v1/metric-sets/{id}`、`DELETE /api/v1/metric-sets/{id}`）属于写操作，一期仅走前端 UI（指标集管理页），与 R9-03 写 API 策略一致；二期随写 API 开放同步提供 REST 端点。

#### 4.11.3 查询执行管线与方言适配（下推逻辑）
`POST /query` 经统一管线，保证"口径唯一、权限受控、下推高效"：
1. **校验**：指标码存在性 + 维度合法性 + 粒度边界（4.5）+ 权限（4.9）+ 配额（4.11.9）；任一不通过即拒绝，不进入执行。
2. **路由决策**：依命中情况选执行路径——
   - 命中物化视图/聚合表 → 直接查预聚合（最快，呼应 4.5 生产数仓落地）；
   - 未命中但有有效缓存 → 按 `stale` 策略返回（4.11.7）；
...
#### 4.11.5 查询结果缓存一致性规范 [R3-08] [R4-02]
- **缓存 Key 设计**：`{metric_code}:{metric_version}:{dimensions_sorted}:{filters_hash}:{dateRange}:{serving_mode}`——含版本号保证口径切换后自动失效，含 serving_mode 区分批/实路径。
- **失效触发机制**：分区重算→按 `metric_code + dateRange` 精确淘汰（呼应 4.5 数据回刷）；口径版本升级→该指标全量 Key 淘汰；维度值变更→涉及维度值的所有 Key 淘汰；质量异常标记→不淘汰但后续命中返回 `quality_downgraded` meta。
- **分布式一致性**：多 API 实例共享 Redis 缓存，失效指令经 Redis Pub/Sub 广播至所有实例（最大延迟 < 1s）；淘汰操作幂等（重复淘汰不影响正确性）。
- **缓存 TTL**：默认 TTL = 该指标 `freshness` 周期（T+1 指标 TTL 24h，小时级 TTL 1h），过期后 stale-while-revalidate（4.13.3）。
- **缓存命中率**入 4.10 运营看板，低命中率驱动 4.5 预聚合优化。
- **ES 索引一致性 [R8-02]**：指标变更（名称/标签/域/状态变更）除触发 Redis 缓存失效外，同步更新 ES 索引文档——① 变更写入 MySQL 后经索引同步器异步更新 ES（延迟 ≤ 5min）；② 指标 DEPRECATED 后 ES 文档须标 `status=DEPRECATED` 且搜索默认过滤（仅 `accept_deprecated=true` 时返回）；③ ES 与 MySQL 一致性纳入每日对账（R8-01），差异修复以 MySQL 为准。
- **缓存 vs 图库同步独立路径 [R8-07]**：指标变更同时触发两条异步路径——① Redis 缓存失效（Pub/Sub，延迟 < 1s）；② Neo4j 图数据同步（图库同步器，延迟 ≤ 5min）。两者独立运行、独立延迟窗口，消费方应按场景选择数据源：实时指标值查询走 Redis+MySQL（缓存失效后即刻取新值），血缘图/影响面分析走 Neo4j（容忍分钟级延迟）。若 Redis 已刷新但 Neo4j 仍旧，血缘图数据可能短暂滞后于指标值——前端血缘图须标注"数据同步延迟，最后同步时间 <timestamp>"。

#### 4.11.6 试算沙箱（dry-run）
- `POST /query/dry-run`：对指定样本分区跑口径，返回样例结果与耗时/扫描行数，供注册/改口径时 Owner 比对；**不计费、不写生产、不进缓存**。
- 用于 4.3 预检升级（新口径与旧口径同分区对齐）、4.8 质量规则预校验。

#### 4.11.7 结果语义标注体系（meta 标准字段）
...
- `serving_mode`：本次结果的数据路径（`batch`/`realtime`），批流双路指标消费方须知道拿的是哪条路径的数据 [R13-14]。

#### 4.11.8 消费侧冲突运行时检测（闭环呼应 4.7）
...
#### 4.11.9 鉴权、限流与资源保护（消费侧合规与稳定）
- **Token 模型**：消费方申请 `api_client_id/secret`，平台签发短时效 Bearer(JWT)，含 `client_id/scope/tenant_id/metric_whitelist/TTL`。[R7-05] **`tenant_id` 语义**：本平台仅支持私有化部署（4.15.8），`tenant_id` 不表示 SaaS 多租户隔离，而是"接入应用/消费方分组标识"——用于区分同一部署实例下不同接入方（如 QuickBI 实例、DataAgent 实例、内部报表系统）的配额与审计归因。全文中"白名单租户"（4.3a/4.5 灰度）统一语义为"白名单接入方"。
- **权限对齐**：token 范围 = 申请时绑定角色 × 主题域 × 指标白名单（4.9），不绕过、DataAgent 不豁免；越权返 `403` 并审计（4.10）。[R7-03] **指标状态实时校验**：token 的 `metric_whitelist` 在签发时静态绑定指标码，但指标状态可能变更（DEPRECATED/DATA_SOURCE_DROPPED）——平台不主动吊销存量 token（TTL 内仍有效），但 PDP 判定时实时检查指标状态：① DEPRECATED 指标须消费方显式传 `accept_deprecated=true`，否则返回 `FORBIDDEN_DEPRECATED`（附录 A）；② DATA_SOURCE_DROPPED 指标返回 `METRIC_SOURCE_DROPPED`（410），引导联系 Owner 或使用历史快照；③ token 过期续签时自动剔除已废弃指标码，收敛攻击面。
...
- **API 版本策略**：
  ...
  - 消费方适配：API 控制台展示"当前版本/即将废弃版本/迁移指南"，`platform_admin` 可查询仍使用旧版本的 `api_client` 列表并定向通知。[R10-06] **消费方主动检测**：消费方登录 API 控制台时，若其 `api_client` 仍在使用即将废弃（Sunset 期内）或已废弃的 API 版本，顶部显示黄色/红色横幅提示"您正在使用即将废弃的 API 版本 v1，请于 <date> 前迁移至 v2"（附迁移指南链接）；已废弃版本调用响应 Header 含 `Sunset` + `Deprecation`，消费方可据此程序化检测。
...
#### 4.12.3 鉴权、限额与降级
- **鉴权**：MCP/NL2SQL 端点强制携带 4.11 的 API token（绑定主体+域+指标白名单+TTL，见 4.9.10），无 token 或越权指标返回 403（呼应 4.9）。
...
- **舱壁隔离（Bulkhead）**：LLM 池/计算池/图库池资源隔离，单依赖拖垮不影响其他能力。[R8-09] **舱壁与 6.4 服务划分映射**：① LLM 池 = 独立 Worker 实例组（消费 Redis 队列 LLM 通道），与物化 Worker 隔离；② 计算池 = API 内 DataSource 连接池（受 `data_source.quota` 约束，每 DataSource 独立配额）；③ 图库池 = 图库同步器实例 + Neo4j 只读连接池（查询走只读副本，同步器走 Leader 写入）。三池资源互不争抢，单池打满不影响其他池可用性。
...
- **事件源接入**：审核待办（4.5）、冲突仲裁（4.7）、质量告警（4.8）、消费侧冲突（4.11.8）、API 异常（4.11.9）、能力降级（4.13）、权限变更（4.9）统一经事件总线入本中心。
...
#### 4.15.6 密钥、加密与不出域
- **逻辑**：静态加密（存储层 AES-256/KMS）、传输 TLS 1.3；平台**不持有源库密钥**（脱敏还原走源权限）；私有化部署数据不出域（第 9 章约束），仅读取与语义编排；导出受 4.9 权限 + 行数上限 + 脱敏 + 4.10 审计（呼应 4.11.9）；Agent（4.12）所见维度值受脱敏，PII 不出域。
- **失败分支**：KMS 不可达 → 密钥轮转暂停并告警，不降级为明文；出域请求（如外部 Webhook 带 PII）被 PEP 拦截。
- **能力边界**：平台不持源密钥，密文还原不在平台能力内；跨境须法务评估（非平台判断）。

#### 4.15.7 留存、审计与被遗忘权
- **逻辑**：审计 WORM 留存（≥180 天热存后冷归档，4.10）；PIPL 撤回同意/销户时，审计中个人标识脱敏或泛化擦除（保留事件不留可识别个人，4.10）；PII 字段访问/推导单独留痕满足最小可用与留存期限。[R7-06] **被遗忘权与 WORM 的协调**：WORM 约束"行不可删改"，被遗忘权要求"个人标识不可识别"——两者不矛盾，实现方式为**覆写脱敏**（将审计行中 PII 字段如 `actor.name`/`context.IP` 覆写为 `ANONYMIZED_<hash>`），保留事件行但使个人不可识别。禁止物理删除审计行，覆写操作本身入审计（`action=PII_ANONYMIZED`）。
...
└── 管理
    ...
    ├── 审计日志
    ├── 运营看板
    ├── 平台健康仪表盘（API/采集/血缘/LLM/数据源/存储聚合健康）
    └── LLM 模型配置（模型注册/连通性测试/健康看板/Prompt 模板/Golden Set）[R6-07]
```
...
| 指标详情 | 口径定义卡、版本历史、血缘图、质量、消费者、业务关系、消费指南、消费洞察；右侧操作区（关注/反馈/申请权限）[R5-06] | 全员（`viewer`+） |
...
| 指标集 | 指标集定义/编辑/消费（一键出图/出表/出 API） | `metric_owner`/`domain_admin`（消费：`viewer`+） |
| 平台健康仪表盘 | API/采集/血缘/LLM/数据源/存储聚合健康视图 | `platform_admin` / 运维 SRE |
...
| 多指标对比分析 [R3-12] | `viewer`+ | 指标目录/详情页"对比" | 已授权指标 ≥ 2 | 多维度值/多时段并排出图/表，量纲警告，可保存分享 |
| 批量注册 [R3-17] | `metric_owner`/`domain_admin` | 注册向导→选宽表 | 宽表已采集 | N 指标候选可批量勾选提交，维度一次映射，逐条审核 |
| 弱网/离线降级 [R3-30] | 全员 | 自动检测（超时 > 5s） | 弱网/断网环境 | 表单不丢失，搜索退本地索引，血缘降级为表级 |

#### 5.4.2 搜索与找数交互
- **逻辑**：顶栏输入 → 防抖 200ms → 召回（指标名/同义词/术语，4.6 协同检索）→ 分类聚合（指标/术语/页）→ 下拉展示结果卡（口径摘要 + 状态标签 + 新鲜度 meta，呼应 4.11.7）→ 回车/点击进详情；`⌘K` 同逻辑但额外暴露"快捷操作"（跳注册/审核/切域）。[R3-27] **搜索排序策略**——默认排序为"可信度优先"：PUBLISHED > EXPERIMENTAL > DRAFT > DEPRECATED（降权）；同状态内 Tier-1 > Tier-2 > Tier-3；同 Tier 内按健康度降序（5.5.3）；同健康度内按近 30 天消费频次降序（4.11 查询日志）。消费方可切换排序维度（相关度/最近更新/消费热度/健康度），切换状态持久化（5.2.7 用户偏好）。DEPRECATED 指标默认不出现在搜索结果（须高级筛选"含已废弃"才可见）。
...
#### 5.4.3 指标详情与可读口径
- **逻辑**：详情页上方口径定义卡（口径版本 + 计算公式人话描述 + 维度 + 来源表 + 新鲜度 meta）；中部 Tab（版本历史 / 血缘图 / 质量 / 消费者 / 业务关系 / 消费指南，分别对接 4.5/4.4/4.8/4.11/R3-13/R3-20）；右侧操作区（关注→入 4.5 关注订阅、反馈→入 4.7 反馈、申请权限→5.4.7）。[R3-26] **消费方视角补充**——新增"消费洞察"子 Tab 或在消费者 Tab 内扩展：① **常用查询**（"被查询最多的维度组合与时间段"，数据源 4.11 查询日志，TOP 10，助新人快速理解"大家怎么用这个指标"）；② **相关报表**（哪些 QuickBI 报表引用了该指标，报表名+Owner+最近访问时间，数据源 4.11 `CONSUMED_BY` 边）；③ **典型用法示例**（Owner 维编的"推荐用法"卡片，如"按地区+月度看 GMV 趋势"，一键跳转 4.11 `/query` 或 QuickBI）。
...
#### 5.4.3a 多指标/多维度对比分析 [R3-12]
- **逻辑**：分析师常需"同指标不同维度值/时间段对比"或"不同指标并排对比"，在指标目录或详情页提供"对比分析"入口——① **同指标多维度值对比**：选择指标 + 选择维度 + 勾选多个维度值（如地区=华东/华北/华南）+ 时间范围 → 调 4.11 `/query` 返回多组值并排展示（表/图），标注口径版本与新鲜度；② **同指标多时间段对比**：选择指标 + 多个 dateRange（如本月 vs 上月 vs 去年同期）→ 返回多组值对比；③ **不同指标并排对比**：选择 2-5 个指标 + 共同维度 + 时间范围 → 并排展示（要求指标量纲兼容，不同单位/量纲时警告"不可直接比较"）；④ **对比结果可保存/分享**：保存为"对比视图"（含指标/维度/时间/图表配置），可生成分享链接（受 4.9 权限约束，分享方须有所有涉及指标的查看权限）。
- **失败分支**：指标量纲不兼容时返回警告但允许展示（标注"不同量纲，仅供参考"）；对比查询任一指标失败则整体降级，成功部分正常展示。
- **能力边界**：对比为展示辅助，不做自动归因/结论；最多 5 个指标 × 10 个维度值，超限提示拆分；对比视图不版本化（仅供即时消费）。
- **验证标准（AC）**：同指标多维度值可并排出图/表；不同量纲有警告；对比结果可保存/分享；查询失败部分降级不整页崩。
...
#### 5.4.4a 批量注册交互 [R3-17]
一张宽表常产出 5-10 个指标（4.5 宽表拆分配置），单条注册效率低，须支持批量注册：
- **逻辑**：① 选中宽表后，4.3 LLM 一次生成多个指标候选（度量列×聚合组合），以列表形式展示（每行=一个指标候选：名称/口径/维度/置信度），支持逐项编辑与勾选（不注册的取消勾选）；② 维度列批量映射：所有候选指标共享同一维度集，一次性映射到共享维度（而非每个指标重复映射）；③ 提交审核策略——**批量提交、逐条审核**：所有勾选指标一次性提交进入 `PENDING_REVIEW`，但审核台逐条展示（`reviewer` 可逐条通过/驳回，也可批量通过低风险指标）；批量提交生成一个 `batch_id` 关联，审核与审计按 batch 聚合展示；④ **批量注册失败回滚**：按指标粒度独立事务——单指标注册失败不影响其他指标；部分失败时报告失败清单（含失败原因），已成功注册的指标不自动回滚（可批量撤销）。
- **能力边界**：批量注册上限 20 个指标/批次（超限提示分批）；批量注册不跳过任何门禁（每个指标仍须通过 4.5 命名/依赖/粒度/探针校验）；维度映射为批量共享，单指标维度调整需在提交前逐项修改。
- **验证标准（AC）**：宽表一次产出 N 个指标候选可批量勾选与提交；维度映射一次完成非逐条重复；审核台按 batch 聚合但可逐条处理；部分失败不影响已成功项。
...
- **逻辑（灰度）**：审核通过后可选"灰度"——先对白名单租户/报表生效，观察质量与波动（4.8 质量监控 + 4.5 波动看板）；无异常一键全量，异常一键回滚到上一版本（4.5 版本历史）；灰度期指标标"实验中"且消费侧 meta 标注 `experimental`（呼应 4.11.7）。
...
#### 5.4.8 冲突协商与仲裁 [R11-18] 编号重排说明
> 原 5.4.8 拆分为多个子节（8a/8b/8c/8d）导致结构混乱，现将内容重组为连续编号——冲突协商与仲裁回归 5.4.8，API 接入引导与消费方迁移引导独立编号，新增维度管理/术语库/质量监控交互规格补齐缺失。
...
#### 5.4.9 API 接入引导（onboarding）[R10-05, R11-18]
- **逻辑**：消费方从"想用 API"到"首次成功调用"的端到端引导——
  1. **申请接入**：管理→Semantic API 控制台→"新增应用"→ 填写应用名称/用途/期望域范围 → `platform_admin` 审批 → 签发 `api_client_id` + `secret`（托管至 Secret Manager）。
  2. **配置授权**：审批通过后引导页展示"下一步：配置指标白名单"→ 选择域 + 指标（支持按域全选或逐指标勾选）→ 保存后自动生成 `metric_whitelist`。
  3. **连通验证**：引导页提供"测试连通"按钮 → 一键调 `GET /api/v1/metrics?page_size=1` → 成功显示 ✓ + 首次返回的指标摘要；失败显示错误码 + 修复建议。
  4. **获取文档**：引导页提供 API 文档链接 + cURL/Python/Java 示例代码（含 `Authorization: Bearer <token>` 模板）→ 一键复制。
  5. **配额确认**：展示当前 QPS/日调用免费额度与已配置配额 → 可申请调整（走 5.4.7 权限申请流）。
  6. **上线标记**：消费方确认配置完成 → 标记"已上线"，此后调用日志与成本归因可见。
- **失败分支**：审批驳回→通知申请人理由+修改入口；连通失败→错误码+修复建议+重试按钮；白名单为空→提示"至少选一个指标"。
- **验证标准（AC）**：新消费方从申请到首次成功调用 ≤ 30 分钟；引导页 6 步全部可交互完成；连通验证一键可达。

#### 5.4.10 消费方指标迁移引导 [R10-07, R11-18]
- **逻辑**：当消费方关注的指标被 DEPRECATED（4.5 废弃与替代）时，4.14 通知含 successor 迁移指引——通知中提供：① 一键跳转 successor 指标详情页；② 自动检查消费方对 successor 是否有权限（若无→显示"申请权限"按钮，一键走 5.4.7 申请流）；③ 如果消费方用 QuickBI 看板绑定了该指标，通知附"受影响看板列表"（4.11 查询血缘反填），提示更新绑定。一期不支持 QuickBI 看板自动更新绑定（需 QuickBI API 联动），预留二期扩展。
- **失败分支**：successor 指标未指定→通知改为"该指标已废弃，请联系 Owner 确认替代方案"；successor 无权限且申请被拒→消费方可选择继续使用历史快照（`GET /metrics/{id}/snapshots`）。
- **验证标准（AC）**：通知含 successor 跳转链接；权限自动检查+一键申请；受影响看板列表可见。

#### 5.4.11 维度管理交互 [R11-10]
- **逻辑**：维度管理（4.5 维度定义配置）是指标注册的前置依赖，交互须覆盖以下场景——
  1. **共享维度创建**：维度管理→"新建共享维度"→ 填写维度名/层级链（如地区：国家→省→市→门店）/标准维度值（支持批量导入 CSV）/上卷规则 → 保存后独立版本化 → 可被指标引用。
  2. **私有维度晋升**：指标注册时发现某维度仅本表独有→ 注册为私有维度（作用域=本指标）→ 当第二张表也需要该维度时→ 维度管理页提示"晋升为共享维度"→ 确认后原私有定义转为映射规则，全局可见。
  3. **维度映射**：多表字段名不同但语义相同（如 `region`/`province`/`area`）→ 维度映射页选择目标共享维度 + 补源编码→标准值映射规则 → 保存后跨表指标可在同一维度对齐。
  4. **维度值退役**：核心维度值删除/合并时→ 检查近 30 天查询日志是否有消费 → 有消费则生成"维度值退役待办"（30 天通知期→引用清空→删除）；无消费则直接删除。
  5. **维度变更评审**：维度结构变更（层级增删/上卷规则修改/SCD 策略切换）→ 自动跑影响面（4.4 what-if，列出受影响指标及下钻路径）→ `domain_admin` 审批 → 生效后关联指标维度版本快照同步更新。
- **失败分支**：共享维度名冲突→提示"已存在同名维度，是否复用或合并"；晋升时映射规则生成失败→手动配置兜底；变更评审驳回→维度保持旧版本。
- **能力边界**：维度管理不承载指标计算逻辑（仅控制"合法维度集与下钻路径"）；维度值退役不自动更新 QuickBI 看板绑定（需手动更新）；私有维度晋升不可逆（晋升后不可回退为私有）。
- **验证标准（AC）**：共享维度创建含层级链且可被指标引用；私有维度晋升一键可达且映射规则自动生成；维度映射保存后跨表指标可同维度对齐；维度值退役有 30 天通知期；维度结构变更走变更评审且影响面可见。

#### 5.4.12 术语库交互 [R11-11]
- **逻辑**：术语库（4.6）交互须覆盖——
  1. **术语搜索**：顶栏搜索或术语库页搜索→ 输入标准名/别名/拼音→ 结果按概念卡展示（标准名+定义+边界+引用指标数+所属域）→ 点击进入概念卡详情。
  2. **概念卡详情**：标准名/别名/定义/边界排除项/引用该术语的指标列表（按域着色）/术语间关系图（4.6.5）/版本变更历史 → 操作区（编辑/废弃/提冲突）。
  3. **同义词归一**：新建术语命中既有术语别名→ 生成 `glossary_conflict` 候选→ `domain_admin` 裁决（合并/并存/废弃）→ 裁决入审计。
  4. **批量导入**：术语库页→"批量导入"→ 上传 Excel/CSV → 自动去重建议→ 逐条确认/跳过→ 批量入库。
- **失败分支**：术语搜索无结果→引导"创建新术语"或"提术语缺失反馈"；批量导入部分失败→报告失败行+原因，已成功项不回滚。
- **能力边界**：术语库管名不管口径（4.6.7）；术语缺失不阻断指标发布（仅提示/候选）；术语冲突不自动合并（须人裁决）。
- **验证标准（AC）**：术语搜索按标准名+别名召回；概念卡含引用指标列表与关系图；同义词冲突可裁决入审计；批量导入可部分失败不阻塞。

#### 5.4.13 质量监控交互 [R11-12]
- **逻辑**：质量监控（4.8）交互须覆盖——
  1. **质量规则配置**：质量监控页→"新建规则"→ 选择指标/表/字段级 → 选择维度（完整性/准确性/及时性/一致性/唯一性/有效性）→ 选择规则类型（静态阈值/动态基线/同环比/跨源对账）→ 配置阈值/窗口/严重级/启用域 → 保存后自动纳入检测调度。
  2. **异常清单与闭环**：质量监控页→ 异常列表（按严重级排序，支持按域/指标/维度筛选）→ 点击异常→ 详情（异常指标+维度+当前值 vs 阈值+根因下钻入口+修复建议）→ Owner 处理（确认/驳回/转交）→ 复检闭环。
  3. **血缘传导标红**：异常详情页→ 上游标红链路可视化（沿 4.4 血缘高亮受影响下游指标）→ 点击下游指标→ 跳转该指标详情页（质量 meta 标 `quality_downgraded`）。
- **失败分支**：规则配置校验失败（如阈值无效）→ 高亮错误字段+修复建议；异常复检失败→ 重开 QualityIncident 并通知 Owner。
- **能力边界**：质量规则配置不自动修数（4.8.6）；异常仅告警不阻断发布；根因下钻依赖血缘完整性（血缘断链处无法传导）。
- **验证标准（AC）**：质量规则可按六维度+四规则类型配置；异常清单按严重级排序且可筛选；异常详情含根因下钻+修复建议；血缘传导标红可下钻到源表/分区。
...
#### 5.4.16 弱网/离线场景基本降级策略 [R3-30]
数据开发/分析师常在 VPN 弱网环境使用平台，须提供基本弱网容错：
- **表单暂存**：注册/编辑表单内容自动暂存至 localStorage（含 LLM 预填充结果），网络断开后刷新页面不丢失已填内容；恢复后可一键加载暂存继续编辑。
- **搜索离线索引**：前端 IndexedDB 缓存当前域已授权指标的名称/同义词/状态（轻量索引，< 2MB），弱网时退化为本地搜索（仅名称匹配，无同义词扩展，无 LLM 联想），结果标注"离线搜索，结果可能不完整"。
- **血缘图懒加载阈值**：弱网检测（请求超时 > 5s 触发）→ 血缘图自动切换为"仅加载 L1 表级概览"，用户可手动点"加载字段级详情"触发全量加载（带 loading 提示）。
- **能力边界**：弱网降级仅保证"可用"不保证"完整体验"；离线索引为本地快照，不保证与最新数据一致；表单暂存仅在本地，不与服务端同步；弱网检测基于请求超时非网速 API（兼容性好）。
- **验证标准（AC）**：断网后刷新页面表单内容不丢失；弱网搜索可出结果（标注离线）；血缘图自动降级为表级。
...
> 本章节定义指标全生命周期的**状态迁移逻辑**与**健康度量化模型**，是治理驾驶舱（4.5）、待办（5.4.14）、通知（4.14）、审计（4.10）的统一状态源。状态机对齐 4.5，健康度模型对齐 1.4 北极星"可度量的治理"。

#### 5.5.1 指标生命周期状态机
- **状态集合**：`DRAFT`（草稿/预填充未提交）→ `PENDING_REVIEW`（待审核）→ `PUBLISHED`（已发布生效）→ `DEPRECATED`（废弃）；新增 `EXPERIMENTAL`（灰度实验中，呼应 5.4.6：审核通过但仅白名单租户/报表可见）；新增 `DATA_SOURCE_DROPPED`（数据源断连，PUBLISHED 的异常子态，R5-01）。
- **迁移表（事件 × 源态 → 目标态）**：
  | 事件 | 源态 | 目标态 | 触发条件 |
  |------|------|--------|----------|
  ...
  | 紧急发布 | DRAFT / PENDING_REVIEW | PUBLISHED | **紧急快通道**：`domain_admin` 可跳过常规审核环节直接发布，但须满足：(1) 填写紧急原因（监管要求/生产事故/数据泄露）；(2) 24 小时内补审并留 `EMERGENCY_PUBLISH` 审计标记；(3) 补审未通过须立即回滚。紧急发布的指标在详情页标"⚠ 紧急发布待补审"，补审通过后标记清除。[R6-04] **紧急发布不可跳过合规门禁（4.9.5）**：含 PII 的指标紧急发布仍须 `compliance_officer` 复核通过——合规门禁是安全硬线，不可因紧急而豁免；若合规官不可达，该指标仅可按 `INTERNAL` 分级发布（PII 维度值全脱敏），合规复核补审后方可放开 PII 维度可见范围 |
  | 复活 | DEPRECATED | DRAFT | 重新启用须重新走审核 |
  | 源表断连 | PUBLISHED | DATA_SOURCE_DROPPED | 4.2 采集检测到源表 DROP/不可达，自动标记（R3-04④/R3-10①）；非直接 DEPRECATED，避免误退役；生成 Owner 待办（7 天内须处理）[R5-01] |
  | 源表恢复/改绑 | DATA_SOURCE_DROPPED | PUBLISHED | Owner 确认误报（表临时下线恢复）→ 取消标记回 PUBLISHED；或改绑来源表→走 4.5 变更评审后回 PUBLISHED |
  | 源表断连退役 | DATA_SOURCE_DROPPED | DEPRECATED | Owner 判断无法恢复→标 DEPRECATED 并填替代指标，走 4.5 退役流程；逾期 7 天未处理自动升级 `domain_admin` |
- **逻辑**：每次迁移写事件日志（谁/何时/何因）→ 触发 4.14 通知相关方（Owner/消费者/合规）→ 入 4.10 审计；消费侧经 4.11 获取 `metric_version` + 状态 meta，避免绑错版本（呼应 4.7 口径版本冲突）。[R9-01] **并发控制（乐观锁）**：状态迁移基于乐观锁——迁移请求须携带当前 `status_version`（或 `metric_version`）作为 CAS 条件，服务端校验指标当前状态=请求预期源态，否则返回 `409 VERSION_CONFLICT` + 当前实际状态，提示"指标状态已变更，请刷新重试"。典型竞态场景：① Owner 提交审核（DRAFT→PENDING_REVIEW）与 domain_admin 紧急发布（DRAFT→PUBLISHED）并发→后者先执行则前者 409；② Owner 变更评审（PUBLISHED→PENDING_REVIEW）与 4.2 自动标记源表断连（PUBLISHED→DATA_SOURCE_DROPPED）并发→先到先执行，后到 409 须人工判断。所有 409 冲突入 4.10 审计，不静默覆盖。

#### 5.5.2 状态可见性与交互
- **列表/详情**：所有指标列表与详情页顶部显示状态徽标（颜色：DRAFT 灰 / PENDING_REVIEW 橙 / PUBLISHED 绿 / EXPERIMENTAL 蓝 / DATA_SOURCE_DROPPED 紫 / DEPRECATED 红），支持按状态筛选（呼应 5.2/5.3）。
- **消费侧约束**：`DEPRECATED` 指标在 Semantic API 默认不可查（须显式 `accept_deprecated`）；`EXPERIMENTAL` 仅白名单可见且 meta 标 `experimental`（呼应 4.11.7）；`DATA_SOURCE_DROPPED` 指标查询返回 `METRIC_SOURCE_DROPPED` 错误码（附录 A）且 meta 标 `source_dropped=true`，消费方须评估是否仍使用该指标历史值。[R5-01]
- **待办联动**：状态迁移产生的审核/裁决/续期待办进 5.4.14 待办中心，SLA 倒计时驱动 1.4 待办清空率。
...
- **逻辑**：单指标 0–100 分，五维度加权（权重可配，默认如下）：
  ...
  | 活跃度 | 20% | 4.11 消费日志 | 近 30 天查询/消费次数归一化。[R11-02] **消费管线口径**：与 R11-01 对齐，活跃度数据源 = 4.11 查询日志（Semantic API + QuickBI 嵌入 + MCP 工具调用，统一入 `operation_audit`），三条管线消费均计入活跃度 |
...
- **指标表达式语言规范（Metric Expression Language, MEL）**：派生/复合指标的 `derived_expression` 采用统一的 MEL 语法，定义如下：
  ...
  - **语法规范（核心 BNF）**：
    ...
    ```
  - **业务语义函数定义** [R5-13]：
    - `RATE(expr, period)`：环比增长率——`(当前周期 expr - 上一周期 expr) / 上一周期 expr`，`period` 为时间粒度（`DAY`/`WEEK`/`MONTH`）；要求 `expr` 数值型、分母不为零（零时返回 `null` 并标 `division_by_zero` meta）
    - `RATIO(expr, total_expr)`：占比——`expr / total_expr`，`total_expr` 须为同粒度下含 `expr` 的全量值（如 `RATIO(sales.gmv_huadong, sales.gmv_total)`）；结果 ∈ [0,1]，超出时标 `ratio_overflow` 警告
  - **示例**：
  ...
  - **翻译层**：MEL AST → 方言 SQL 生成器（6.1 解析路由），新增数据源类型只需补翻译适配器。[R9-08] **适配器接口契约**：① 输入：MEL AST + DataSource 类型 + `data_source.quota`；② 输出：方言 SQL + 分区谓词（`WHERE dt BETWEEN ...`）+ 扫描行数预估值；③ 注册方式：配置注册（`data_source.type` → 适配器类映射），平台内建 MySQL/PG/Doris/Hive 四种，自定义适配器实现标准接口后配置注入；④ 测试要求：新适配器须过 dry-run 标准测试集（含聚合/过滤/分区裁剪/JOIN 四类用例）方可启用；⑤ 适配器异常（不支持的方言函数/存储过程）返回 `422 EXPRESSION_INVALID` 并标注"方言不支持：<函数名>"。
...
| 实体 | 说明 |
|------|------|
| data_source | 数据源连接与配额（含 `cluster_id` 多集群标记）。[R9-05] **quota 子字段**：`max_scan_rows`（单查询扫描行数上限，默认 10,000,000）、`max_concurrent`（并发查询上限，默认 5）、`query_timeout_seconds`（查询超时秒数，默认 30，联动附录 B）、`cost_per_scan_unit`（扫描单价，元/百万行，用于 4.10 成本核算） |
...
| policy / grant / data_classification / compliance_review | 权限策略（RBAC+ABAC 表达式）/ 细粒度授权（域/指标/维度值+有效期）/ 数据分级标签 / 合规复核记录（4.9.8，R7-04） |
...
| ops_cost | 成本核算（按域/消费方聚合）。[R10-02] **字段结构**：`domain_id`/`api_client_id`（二选一或双填）、`cost_type`（LLM/QUERY/MATERIALIZED/ES）、`cost_amount`（元，按单价×用量估算）、`usage_quantity`（token 数/扫描行数/存储 GB）、`billing_period`（YYYY-MM）、`budget`（月度预算，元）、`free_quota_used`（免费额度已用量） |
| lineage_edge_history | 血缘边变更快照（R10-04）。字段：`edge_id`/`before_source`/`before_target`/`before_transform_expr`/`before_confidence`/`after_source`/`after_target`/`after_transform_expr`/`after_confidence`/`change_reason`/`trigger_event`（schema_drift/reparse/manual）/`changed_at`/`changed_by` |
| metric_business_relation | 指标间业务关系（LEADS/CAUSES/BELONGS_TO_PROCESS，R3-13） |
| metric_set / metric_set_item | 指标集定义与指标成员（R3-15）。[R11-05] **版本化实现**：`metric_set` 内含 `version` 字段（整数，每次指标增删自增），不另建独立版本表——指标集版本化语义较轻（仅记录"哪一版包含哪些指标"），无需像 `metric_version` 那样独立实体。`metric_set_item` 含 `set_id + metric_code + added_version`（该指标在哪一版加入），成员删除不物理删除而是标记 `removed_version` |
| schema_drift_event | Schema 变更检测事件（R3-04） |
| notification / event_log / subscription_pref | 通知中心事件模型、投递日志与订阅偏好（4.14.7，R5-03） |
| dependency_health / degradation_event | 依赖健康状态与降级事件记录（4.13.6，R5-04） |
...
erDiagram
    ...
    lineage_edge ||--o{ lineage_edge_history : "边变更快照（R10-04, R11-04）"
    ...
    rbac_role ||--o{ policy : "策略定义（RBAC+ABAC，R7-04）"
    policy ||--o{ grant : "细粒度授权（指标/维度值+有效期，R7-04）"
    data_classification ||--o{ compliance_review : "分级→合规复核（R7-04）"
    atomic_metric ||--o{ data_classification : "指标分级标注（R7-04）"
    ...
    rbac_role ||--o{ ops_cost : "域级成本归属（R11-06）"
    api_client ||--o{ ops_cost : "消费方成本归属（R11-06）"
    ...
    atomic_metric ||--o{ metric_business_relation : "业务关系（LEADS/CAUSES/PROCESS）"
    metric_set ||--o{ metric_set_item : "指标集成员"
    atomic_metric ||--o{ metric_set_item : "指标集成员引用（R5-05）"
    data_source ||--o{ schema_drift_event : "Schema 变更检测"
    event_log ||--o{ notification : "事件→通知投递（R5-03）"
    dependency_health ||--o{ degradation_event : "依赖→降级事件（R5-04）"
```

关键耦合：① `data_set` 是逻辑表↔物理表唯一桥（含 `cluster_id` 多集群与 `dw_layer` 分层）；② `metric_version` 串联指标/物化/血缘/术语/索引；③ `lineage_edge` 经 `data_source` 与 `atomic_metric` 双锚；④ `api_token`+`operation_audit` 实现 AI 消费可溯；⑤ `llm_model_config` 串联 Prompt/校准/测试，是 4.3a 的存储基础；⑥ 指标实体含 `dw_layer`/`metric_tier`/`serving_mode`/`additivity` 字段驱动差异化治理与消费策略；⑦ `metric_business_relation` 承载指标间业务语义关系（先行/因果/流程），支撑 4.17 推荐与业务认知；⑧ `metric_set`/`metric_set_item` 实现消费侧指标组合；⑨ `schema_drift_event` 记录源表结构变更检测事件，驱动指标联动处理。

### 6.4 部署架构

- **服务划分**：Web（React）、API（FastAPI，多实例无状态）、Worker（LLM 解析/物化调度，消费 Redis 队列）、图库同步器、索引同步器（MySQL→ES 异步同步）。[R8-03] **图库同步器 HA**：同步器为无状态 Worker（消费 Redis 重试队列），多实例部署（≥2），任一实例故障另一实例接管；队列积压超 10,000 条或超 30min 未消费 → 触发 P1 告警（4.14 通知 `platform_admin`）；积压期间消费方读 Neo4j 拿到脏数据 → 血缘图标注"数据可能延迟，最后同步时间 <timestamp>"，超 1h 未恢复 → 降级为 MySQL 直查表级概览（4.13 血缘图渲染降级）。
- **有状态组件 HA**：MySQL（主从）、Neo4j（因果集群）、Redis（哨兵）、ES（集群）；双写最终一致（图库与 MySQL 重试队列）。[R8-01] **双写一致性模型细化**：
  - **写路径**：MySQL 为主写（事务内同步），Neo4j/ES/Redis 为异步同步（经图库同步器/索引同步器/缓存 Pub/Sub）。
  - **失败处理**：MySQL 写成功 → Neo4j/ES 同步失败 → 进重试队列（Redis-backed，持久化不丢），重试指数退避（最大 10 次 / 30min）；超限告警 `platform_admin` + 4.10 审计 + 4.14 通知。
  - **对账机制**：每日凌晨跑全量对账（MySQL 指标/血缘/权限 vs Neo4j 图数据 vs ES 索引文档），对账结果写 `consistency_check_report`（实体类型 + MySQL 记录数 vs Neo4j 节点数 vs ES 文档数 + 差异明细）；差异率 > 0.1% 触发 P1 告警；差异率 > 1% 触发 P0 告警 + 暂停消费侧 Neo4j 查询降级为 MySQL 直查。
  - **修复策略**：MySQL 为权威源（Source of Truth），对账差异以 MySQL 为准修复 Neo4j/ES——自动补写缺失节点/文档、自动删除孤儿节点/文档；修复操作入 4.10 审计（`action=CONSISTENCY_REPAIR`）。
  - **一致性窗口**：正常情况下 Neo4j/ES 与 MySQL 差异 ≤ 5min（同步频率）；重试队列积压时窗口放大，消费方按场景选择数据源（实时查询走 Redis+MySQL，血缘图走 Neo4j 并容忍分钟级延迟）。
...
| Semantic API 单查询 | ≤ 1s（缓存命中），限流 20 QPS/客户端（多客户端并发时集群总 QPS 上限见 6.5a）[R5-02] |
| 批量 LLM 解析吞吐 | 按 golden set 校准，单任务失败不影响全局 |
| 血缘图 L1→L2 展开渲染 P95 | ≤ 3s [R12-14] |
| LLM 解析单任务 P95 | ≤ 30s [R12-14] |
| 物化任务产出延迟 P95 | ≤ 2× 源 ETL 延迟（上限 1h）[R12-14] |
...
**增长预估公式 [R8-08]**：每注册 1 个指标约增加：① MySQL 行数：3–5 行（指标定义 1 + metric_version 1 + lineage_edge 1–3）；② Neo4j 节点：2–4 个（Metric + MetricVersion + LineageEdge）+ 边 3–6 条；③ ES 文档：1 条（指标检索文档）；④ 审计日志：3–8 条（注册/审核/发布事件）；⑤ Redis Key：0（查询时懒加载缓存）。据此推算：指标数从 2,000 → 5,000 时，MySQL 行数约增 9K–15K，Neo4j 节点约增 6K–12K，ES 文档约增 3K。运维据此预判扩容时点，而非仅靠阈值告警被动响应。
...
- **上线方案**：
  ...
  - **数据迁移**：历史 ETL 元数据经 4.2 采集补录；存量指标由 Owner 确认后入库。[R8-05] **存量迁移质量校验**：① 迁移后指标默认标 `DRAFT`（非直接 PUBLISHED），须 Owner 确认口径 + 绑定数据源后走审核发布；② 存量指标无 Owner → 由 `domain_admin` 指定临时 Owner（30 天内须认领，逾期标"待认领"告警）；③ 迁移批次策略：按域分批（每批 ≤ 200 指标），每批完成后跑 4.8 质量规则抽样校验（校验通过率 ≥ 90% 方可继续下批）；④ 迁移进度与质量在运营看板可见。
  - **应急预案**：LLM/计算/DataSource 故障按 4.13 降级；重大故障熔断消费侧。
  - **备份恢复与 RTO/RPO [R8-04]**：
    - **备份策略**：MySQL（binlog + 每日全量备份，保留 7 天）、Neo4j（每日 dump，保留 3 天）、Redis（AOF + 每日 RDB 快照）、ES（每日 snapshot，保留 3 天）；备份存储于对象存储，跨可用区冗余。
    - **RTO/RPO 目标**：MySQL RPO ≤ 5min（binlog 实时归档）/ RTO ≤ 30min（从库提升 + 全量恢复）；Neo4j RPO ≤ 24h（因果集群 Raft 日志）/ RTO ≤ 1h（集群重选举）；Redis RPO ≤ 1min（AOF fsync 1s）/ RTO ≤ 10min（哨兵故障转移）；ES RPO ≤ 24h / RTO ≤ 1h。
    - **恢复演练**：每季度一次备份恢复演练（类似 4.13 GameDay），验证 RTO/RPO 达标；演练结果入 4.10 审计。
    - **灾备切换**：MySQL 主库不可达 → 从库自动提升（哨兵/Orchestrator），切换时间 ≤ 30s；Neo4j Leader 不可达 → 因果集群自动重选举，切换时间 ≤ 60s；切换期间写入暂阻塞，查询走只读副本。
...
| 期 | 时间 | 范围 | 关键交付 |
|----|------|------|---------|
| 一期 MVP | W1–W7 | 采集 + 血缘 + 语义建模 + 执行引擎 + 冲突检测 + RBAC 骨架 + Semantic API 验证出口 + Schema Drift 检测 + 批量权限操作策略 + PII 指标合规复核门禁（4.9.5）+ 口径漂移巡检（4.7.8）[R11-13] | 平台首次可用，跑通"采集→注册→冲突→消费"闭环 |
| 一期完整 | W8–W10 | LLM 解析 + 术语库 + 质量 + QuickBI 嵌入 + 审计度量 + 降级 + 模板包/帮助中心 + 合规门禁 + 消费指南 + 指标集 + 多指标对比分析 + 批量注册 + 平台健康仪表盘 + 弱网降级 [R11-14 补齐] | 治理与消费完整闭环 [R5-07] |
| 二期 | W11–W16（待容量评估确认） | 行级权限 RLS（4.9.4 二期深化，R7-07）、治理驾驶舱深化、反馈驱动迭代、FR-19 智能找数推荐 | 治理精细化与轻量消费 |
...
> **各期退出标准**：一期 MVP 退出 = 4.2/4.4/4.5/4.6/4.7/4.9(骨架)/4.11 验收通过（9.1 对应项）；一期完整退出 = FR-03(4.3)/FR-08(4.6)/FR-10(4.8)/FR-12(4.11 QuickBI)/FR-16(4.10)/FR-17(4.13)/合规门禁全量可用 [R12-15]；二期退出 = 行级权限上线 + 治理驾驶舱与健康度红黑榜可用；三期退出 = 血缘准确性门禁生效；四期退出 = NL2SQL 与 DataAgent MCP 限流/降级可用。
...
| 模块 | 验收要点 | 分期归属 [R12-15] |
|------|---------|-------------------|
| 元数据采集 | 自动扫库建元数据；PII 字段自动分级标注；**Schema Drift 检测与影响分析（破坏性变更生成待办，R3-04）**。**[R12-01] 量化标准**：① 自动采集覆盖率 ≥ 95%（已配置数据源的表/字段入库率）；② PII 标注准确率 ≥ 90%（抽样 100 字段人工复核）；③ Schema Drift 端到端测试：注入已知 DDL 变更→检测→生成待办→Owner 确认，全链路 ≤ 10min | MVP |
| LLM 解析 | golden set 校准 ≥ 0.85 方可全量；失败进重试；**Prompt 模板灰度 + golden set 校准闭环（4.3a）**。**[R12-02] 量化标准**：① golden set 冷启动最低样本数 ≥ 50 条（覆盖 3 类指标 × 3 种能力）；② 初始 golden set 由 `platform_admin` + 首域 `domain_admin` 人工编制，首域上线前完成；③ golden set 每月增量 ≥ 10 条 | 一期完整 |
| 血缘 | 表级 + 字段级双通道；影响面分析准确；**调度依赖↔数据血缘双向校验（R3-05）**。**[R12-03] 量化标准**：① 影响面分析端到端测试场景：预置 10 指标 × 5 表血缘图→删除指定源字段→影响面输出含预期 3 个下游指标 + 2 个报表→判定通过；② 影响面召回率 ≥ 95%、精确率 ≥ 90% | MVP |
| 语义建模 | 原子/派生/复合三类可定义；版本化生效；**PENDING_VERSION 消费侧行为规范（版本钉住 30d Sunset，R3-01）**。**[R12-04] 端到端验证场景**：指标 v1 PUBLISHED→创建 v2 PENDING_VERSION→v1 消费方仍获 v1 结果→v2 审核通过 PUBLISHED→v1 进入 Sunset 30d→消费方未升级仍可获 v1（带 Sunset 标注）→30d 后 v1 不可消费→全链路可验证 | MVP |
| 执行引擎 | 口径→SQL 下推；物化版本绑定正确；**缓存一致性规范（Key 含版本号/失效 Pub/Sub/TTL=freshness，4.11.5）**。**[R12-05] 端到端验证场景**：指标 PUBLISHED→物化任务产出→API 查询命中物化→返回 `meta.metric_version` 与当前版本一致→口径变更→物化重算→新版本号更新→缓存失效→再次查询获新版本结果 | MVP |
| 指标管理·状态机闭环 [R12-06 拆行] | 状态机闭环（含 DATA_SOURCE_DROPPED 异常子态）；**DATA_SOURCE_DROPPED 逾期 7 天未处理→自动升级 `domain_admin`→升级通知可达→驾驶舱标红可见（R4-01）**；Tier-1 指标结果快照 WORM 落库可回溯；消费指南 Tab（适用/不适用/误用，R3-20） | MVP |
| 指标管理·消费侧能力 [R12-06 拆行] | 待办聚合；关注推送；反馈纠错；SLA 例外日历生效；指标集定义与消费（R3-15）；批量注册交互（宽表→N 指标，R3-17） | 一期完整 |
| 冲突检测 | 同名/相似识别；协商→裁决→记录闭环；**口径漂移巡检（源头 SQL 变更 24h 内检出并告警）**；**业务关系建模（LEADS/CAUSES/PROCESS，R3-13）**。**[R12-07] 端到端测试场景**：注册两个同名异口径指标→系统检出硬冲突→生成双方待办→协商记录留痕→超时升级仲裁→仲裁锁定编辑→全链路可验证 | MVP |
| 质量 | 异常分级；告警触达 Owner + 关注者；**外部基准对账（导入幂等、差异闭环、报告可导出）**；**延迟通知按 dw_layer×metric_tier 差异化策略（R3-03）**；**口径一致性对账调度策略（R3-06）**。**[R12-08] 量化标准**：① 外部基准对账端到端验证：导入基准 Excel→绑定指标→自动比对→差异超阈值生成告警→Owner 72h 内确认→结论留痕→对账报告可导出（PDF/Excel）；② 基准导入幂等验证：同一文件导入 2 次不产生重复记录 | 一期完整 |
| 权限 | RBAC × 域授权；PII 指标合规复核门禁；**批量权限操作策略（dry-run+逐条审计+失败回滚，R3-07）**；**PII 定期重扫与分级变更审计（R7-01/R7-08，4.15.9 重扫 + 4.10.1 分级变更事件）**。**[R12-09] 量化标准**：① PII 定期重扫频率 ≥ 每周 1 次（附录 B `pii.rescan_interval_days=7`）；② 重扫后分级变更审计条目 100% 含 before/after（与 9.3 对齐）；③ 批量权限 dry-run 预览受影响用户数/指标数准确，与实际执行差异 ≤ 0 | MVP（骨架）+ 一期完整（批量+重扫） |
| 消费 | QuickBI 嵌入出图带口径版本；Semantic API 鉴权限流；**多指标/多维度对比分析可保存分享（R3-12）**；**指标集一键出图/出表/出 API（R3-15）**；**API 接入 onboarding 端到端可达（新消费方申请→签发→首次调用≤30min，R10-08）**；**API 文档+示例代码可用**；**消费方可查看自身调用日志与成本归因**。**[R12-10] 验收前提与方法**：① QuickBI 嵌入验收须提供 QuickBI 测试实例（SaaS 或私有部署），联调环境就绪；② API 文档验证：文档页可访问 + curl 示例一键执行成功 + 响应结构与 4.11.2 规范一致；③ onboarding 计时起点=申请接入，终点=首次成功调用 `GET /api/v1/metrics` 返回 200 | MVP（Semantic API）+ 一期完整（QuickBI+onboarding） |
| 降级 | LLM/计算/DataSource 故障优雅降级，不白屏不超时；**弱网/离线降级（表单暂存/离线搜索/血缘懒加载，R3-30）**。**[R12-11] 量化定义**：① "不白屏"= 前端任何依赖故障时 ≤ 3s 内显示降级提示（非空白页）；② "不超时"= 降级路径响应 P95 ≤ 5s；③ 端到端降级测试场景：模拟 LLM 不可用→前端 AI 按钮悬停提示可手动兜底 ≤ 3s；模拟 DataSource 不可用→API 返回 503 + 预计恢复信息 ≤ 5s | 一期完整 |
| 运维 | **平台健康仪表盘（API/采集/血缘/LLM/数据源/存储聚合健康，R3-16）**；**成本归因看板可用（域/消费方月度成本趋势+TOP 成本指标）；预算 80% 预警通知触达+预算 100% 自动限流生效（R10-09）**。**[R12-12] 验收前提**：① 成本归因看板可在一期上线后 T+30 天完成验收（需至少 1 个月运营数据）；② MVP 退出标准中成本看板为"功能可用"非"数据丰富"——看板框架渲染正确 + 虚拟归因字段可填充 + 预算预警通知可触发即可；③ 一期完整退出时成本看板须含至少 1 个月趋势数据 | 一期完整 |

### 9.2 业务成果验收（北极星联动）

- 分析师确认口径平均耗时 ≤ 10 分钟（UAT 抽样）。**[R12-13] 验收方法**：平台埋点（4.14）自动记录"搜索→打开详情→关闭详情"时长，UAT 抽样 20 位分析师首次使用场景，取 P50 为准；基线=上线前人工确认口径的典型耗时（问卷/访谈采集）。
- 月度新增跨域同名不同义冲突下降 ≥ 50%。**[R12-13] 验收方法**：基线=上线前 1 个月跨域同名异口径指标数（4.7 历史快照或人工盘点），对比=上线后第 2 个月新增冲突数；平台 4.7 冲突检测表自动统计。
- 识别并合并重复指标 ≥ 30%。**[R12-13] 验收方法**：基线=上线前全量同名异口径指标数（4.7 冲突检测），合并数=上线后 3 个月内经仲裁合并的指标数；平台 4.7 仲裁记录表统计。
- 口径纠错周期 ≤ 3 个工作日。**[R12-13] 验收方法**：平台待办时效（4.10）自动记录"口径纠错待办生成→闭环"时长，取 P50；须排除 Owner 请假等非平台因素。
- 元数据覆盖率 ≥ 95% 且 80% 核心指标 PUBLISHED。**[R12-13] 验收方法**：元数据覆盖率 = `db_catalog` 已采集表数 / 全量表数（全量表数须先人工盘点或按数据源注册信息统计）；PUBLISHED 率 = 状态 PUBLISHED 的指标数 / 全部已注册指标数；两项均由平台统计页自动计算。

### 9.3 非功能验收

- 性能满足第 6.5 节 P95。
- 可用性 ≥ 99.5%；降级场景不阻断。
- 安全审计覆盖 PII 访问；注入防护生效（四期）。[R7-09] **安全审计细化验收**：① PII 访问审计条目 100% 含 `data_classification=PII` 标记；② 数据分级变更（4.10.1 新增事件）审计条目 100% 含 `before`/`after`（尤 PII 降级须留痕）；③ 被遗忘权执行后审计行中个人标识不可识别（覆写脱敏，4.15.7）；④ DEPRECATED/DATA_SOURCE_DROPPED 指标 PDP 实时拦截生效（4.11.9，R7-03）
- 可观测看板可用，降级事件可告警。
- **多存储一致性与备份恢复验收 [R8-06]**：① MySQL-Neo4j 每日对账差异率 ≤ 0.1%；② ES 索引与 MySQL 指标状态同步延迟 ≤ 5min；③ DEPRECATED 指标在 ES 搜索结果中默认过滤且标 `status=DEPRECATED`；④ 备份恢复演练 RTO/RPO 达标（6.7 定义）；⑤ 图库同步器积压告警生效。

### 9.4 端到端冒烟测试路径（MVP 闭环验证）[R12-16]

> 本节定义 7.1 MVP 关键交付"采集→注册→冲突→消费"闭环的端到端冒烟测试路径，每步有预期结果与判定标准。MVP 退出时此路径必须全链路通过。

| 步骤 | 操作 | 预期结果 | 判定标准 |
|------|------|---------|---------|
| ① 配置数据源 | `platform_admin` 添加 MySQL 数据源连接 | 保存即刻触发全量采集，`db_catalog` 入库表/字段 ≥ 1 张表 | 采集覆盖率 ≥ 95%，PII 字段自动标注 |
| ② PII 自动标注 | 采集含 `id_card`/`phone` 字段的表 | 字段 `sensitivity_level` 自动标 `PII`，血缘图标红 | PII 标注准确率 ≥ 90%（R12-01） |
| ③ LLM 解析指标候选 | 触发 LLM 解析任务 | 候选指标生成，置信度 ≥ 0.85 的直接进草稿 | golden set 校准 ≥ 0.85（R12-02） |
| ④ Owner 确认注册 | `metric_owner` 在注册向导确认候选→提交审核 | 指标进入 PENDING_REVIEW 状态 | 一等字段（粒度/单位/聚合/时间语义）齐全 |
| ⑤ 审核通过 PUBLISHED | `reviewer`/`domain_admin` 审核通过 | 指标进入 PUBLISHED 状态，ES 索引同步可搜索 | 状态机闭环（5.5.1），ES 同步延迟 ≤ 5min |
| ⑥ 消费方申请 API | 新消费方申请接入→签发 `api_client_id`+`secret` | 引导页 6 步可交互完成，连通验证一键成功 | 首次成功调用 ≤ 30min（R10-08） |
| ⑦ 首次查询成功 | 消费方调 `GET /api/v1/metrics` | 返回 200 + 指标摘要 + `meta.metric_version` + `meta.freshness` | 响应 P95 ≤ 1s（缓存命中）或 ≤ 3s（下推） |
| ⑧ 指标变更→影响面分析 | Owner 修改口径→触发影响面 what-if | 影响面输出含受影响下游指标 + 报表 + 消费方 | 影响面召回率 ≥ 95%，精确率 ≥ 90%（R12-03） |
| ⑨ 注册同名异口径指标→冲突检测 | 第二人注册同名异口径指标 | 系统检出硬冲突→生成双方待办→协商记录留痕 | 综合分 < 0.6 判硬冲突（4.7.2） |
| ⑩ 版本升级→Sunset | 指标 v2 PUBLISHED→v1 进入 Sunset 30d | v1 消费方仍可获 v1（带 Sunset 标注），30d 后 v1 不可消费 | 版本钉住 + Sunset 端到端可验证（R12-04） |
| ⑪ Schema Drift 检测 | 源表 ALTER COLUMN（类型变更） | 增量采集检测到破坏性变更→生成"Schema 变更待确认"待办 | 全链路 ≤ 10min（R12-01） |
| ⑫ DATA_SOURCE_DROPPED 闭环 | 源表 DROP→指标标 DATA_SOURCE_DROPPED | 逾期 7 天→自动升级 `domain_admin`→驾驶舱标红 | 升级通知可达（R4-01/R12-06） |
...
| EXPERIMENTAL / DATA_SOURCE_DROPPED | 灰度实验中 / 数据源断连（PUBLISHED 的异常子态，R5-01） |
...
| 指标集（Metric Set） | 消费侧指标组合，一键出图/出表/出 API（R3-15，R5-09） |
| 业务关系（Business Relation） | 指标间语义关系：LEADS（先行）/CAUSES（因果）/BELONGS_TO_PROCESS（同流程）（R3-13） |
| Schema Drift | 源表结构变更检测（增删列/改类型/删表/重命名），联动指标体系（R3-04） |
| 消费指南（Consumption Guide） | 指标消费侧使用手册（适用/不适用/常见误用），由 Owner 编写（R3-20） |
| 批量权限操作 | 多用户×多资源的批量授权/回收，含 dry-run 预览与逐条审计（R3-07） |

### 11.2 开放问题与决策待定项

当前所有已识别问题均已落盘；下表登记已识别但**有意 deferred（排期待定）** 的产品/交互缺口（来源 5.4.20），与"任何新需求须登记"的治理原则自洽。保留此节用于记录后续评审产生的新问题，避免"口头决策"导致治理断点。
...
| OP-12 | R3-21/R3-22：第三轮审查时误判 4.7.8 口径漂移巡检与 SLA 例外日历缺失，实际已在上轮补入正文 | **已确认存在** / 无需补入 | 产品 | 4.7.8 口径漂移巡检已存在（line 811）；SLA 例外日历已存在 4.5（line 672） |
| OP-13 | "数据产品"概念（Data Mesh 产品打包交付） | **已识别** / Deferred（二期/三期） | 产品 | 已预留设计空间（R3-19），一期仅实现指标集雏形 |
| OP-14 | R5-01 DATA_SOURCE_DROPPED 状态机补入及 R6-01 ASCII 图同步更新 | **已闭环** / 已补入 | 产品 + 数据治理 | 已补入 5.5.1 迁移表（3 条路径）+ ASCII 图（6 状态），可见性/消费侧约束已声明 |
| OP-15 | R7 安全审计：compliance_officer 域范围 / 分级变更审计事件 / 废弃指标 token 联动 / WORM vs 被遗忘权 | **已闭环** / 已补入 | 安全 + 合规 | compliance_officer 明确跨域（R7-02）/ 分级变更入审计枚举（R7-01）/ PDP 实时校验指标状态（R7-03）/ 被遗忘权明确覆写脱敏非删除（R7-06） |
| OP-16 | R8 多存储一致性：双写模型 / ES 索引一致性 / 备份恢复 RTO·RPO / 存量迁移质量校验 | **已闭环** / 已补入 | 运维 + 数据治理 | 双写对账机制+修复策略已定义（R8-01）/ ES 索引失效规范已补（R8-02）/ RTO/RPO 已量化（R8-04）/ 存量迁移默认 DRAFT+分批校验（R8-05） |
| OP-17 | R9 配置治理与并发：状态机乐观锁 / 写 API 能力边界 / 配置参数集中清单 / 方言适配器契约 | **已闭环** / 已补入 | 运维 + 工程架构 | 乐观锁 CAS 策略已定义（R9-01）/ 写 API 分期声明已补（R9-03）/ 附录 B 配置清单已建（R9-06）/ 适配器接口契约已定义（R9-08） |
| OP-18 | R10 成本·血缘·消费方体验：共享成本分摊 / 免费额度 / 血缘断链可视化 / 血缘边变更快照 / API 接入 onboarding / 消费方迁移引导 / 成本验收 | **已闭环** / 已补入 | 产品 + 运维 + 前端 | 共享成本一期不分摊+免费额度+虚拟归因（R10-01）/ 断链虚线框+登记入口（R10-03）/ API onboarding 6 步引导（R10-05）/ 消费方迁移引导（R10-07）/ 成本看板+预算预警验收（R10-09） |
| OP-19 | R11 全量审查：消费口径对齐 / lineage_edge_history 实体 / 免费额度错误码 / 维度管理·术语库·质量监控交互 / 分期交付范围对齐 / 配置参数补齐 / 5.4 编号重排 | **已闭环** / 已补入 | 全域 | 消费口径=API+QuickBI+MCP统一入审计（R11-01/02）/ 6.2+6.3 补实体与ER（R11-04/05/06）/ 错误码补FREE_QUOTA_EXCEEDED+BUDGET_EXCEEDED（R11-07/09）/ 5.4 补维度管理+术语库+质量监控交互（R11-10/11/12）/ 7.1 MVP+完整范围与9.1对齐（R11-13/14）/ 附录B补4参数（R11-15/16/17）/ 5.4编号重排8a-d→8-20连续编号（R11-18/19） |
| OP-20 | R12 端到端验收闭环：9.1量化标准缺失 / 9.2验收方法缺失 / 9.4冒烟测试路径缺失 / 6.5性能覆盖不足 / 7.1 FR编号缺4.x引用 / 附录B缺PII重扫参数 | **已闭环** / 已补入 | 验收+运维 | 9.1增分期归属列+12项量化标准+指标管理拆行（R12-01~12）/ 9.2补5条验收方法（R12-13）/ 9.4新增端到端冒烟测试12步路径（R12-16）/ 6.5补3项性能指标（R12-14）/ 7.1退出标准FR编号补4.x引用（R12-15）/ 附录B补pii.rescan_interval_days（R12-17） |
| OP-21 | R13 API 契约精审：4.11.2端点参数不完整 / 响应Schema缺失 / 排序过滤缺失 / 幂等性未声明 / comparison/orderBy结构未定义 / 血缘查询缺参数 / meta缺serving_mode / FORBIDDEN_DEPRECATED vs METRIC_DEPRECATED语义冲突 / 通用Header未定义 | **已闭环** / 已补入 | API+消费层 | 4.11.2补通用Header+排序/过滤参数+响应Schema+批量指标详情端点+血缘查询参数+幂等声明+comparison/orderBy结构+维度交集计算声明（R13-01~10/12/13/15）/ 4.11.7补serving_mode（R13-14）/ 附录A补FORBIDDEN_DEPRECATED vs METRIC_DEPRECATED区分（R13-11） |
...
- 所有错误响应体统一格式：`{"error_code": "<CODE>", "message": "<人类可读描述>", "retry_after": <秒|null>, "trace_id": "<追踪ID>"}` [R6-05] **同一 HTTP 状态码可对应多个 `error_code`**，这是有意设计——消费方应按 `error_code`（而非 HTTP 状态码）路由处理逻辑。例如 `410` 同时用于 `API_VERSION_SUNSET`（API 协议废弃）与 `METRIC_VERSION_SUNSET`（口径版本钉住过期）及 `METRIC_DEPRECATED`（指标退役）和 `METRIC_SOURCE_DROPPED`（数据源断连），四者语义不同、处理方式不同，消费方须按 `error_code` 区分。
...
| `FORBIDDEN_DEPRECATED` | 403 | 访问已废弃指标且未带 `accept_deprecated` | 使用 successor 指标或显式声明 | [R13-11] **与 `METRIC_DEPRECATED`(410) 的区分**：`FORBIDDEN_DEPRECATED`(403)=消费方有权限但未带 `accept_deprecated=true` 请求头，是**权限策略拒绝**（指标仍在 Sunset 期，加头仍可用）；`METRIC_DEPRECATED`(410)=指标已彻底退役(GONE)，无论是否带 `accept_deprecated` 都不可用，须使用 successor |
| **请求校验** | | | |
...
| `BATCH_QUOTA_EXCEEDED` | 422 | 批量注册超上限（20 个/批次，R3-17）或批量权限操作超限 | 拆分为多个批次 [R5-12] |
| `BATCH_PARTIAL_FAILURE` | 207 | 批量操作部分成功部分失败（R3-07/R3-17） | 检查响应体中每条结果，对失败项重试 |
| **限流与降级** | | | |
| `RATE_LIMITED` | 429 | 调用速率超 QPS/RPM/TPM 配额 | 按 `retry_after` 指数退避重试 |
| `FREE_QUOTA_EXCEEDED` | 429 | 月度免费额度已用尽（API 10,000 次/月或 LLM 1,000 次/月，R10-01） | 等待下月重置或申请提升配额；响应含 `budget_reset_at` 字段（R11-07） |
| `BUDGET_EXCEEDED` | 429 | 月度预算已超 100% 自动限流（4.10.2，R11-09） | 等待下月预算重置或申请提升预算；响应含 `budget_reset_at` 字段 |
...
| `METRIC_VERSION_SUNSET` | 410 | 指标口径钉住版本已过 Sunset 期（R3-01，默认 30 天） | 升级至当前口径版本 [R5-11] |
| `METRIC_DEPRECATED` | 410 | 指标已退役（GONE） | 使用 successor 指标 |
| `METRIC_SOURCE_DROPPED` | 410 | 指标数据源已断连（源表 DROP/不可达，R5-01） | 检查 meta.source_dropped，联系 Owner 确认处置或使用历史快照 [R5-10] |
| **LLM 与 AI** | | | |
...
本附录集中管理平台所有可配参数的默认值、可调范围与影响模块，供运维一站式查阅与调整。配置变更均入 4.10 审计（R9-07）。

| 参数名 | 默认值 | 可调范围 | 影响模块 | 配置方式 |
|--------|--------|---------|---------|---------|
| **指标生命周期** | | | | |
| `metric_version.sunset_days` | 30 | 7–90 | 4.5 口径版本钉住 Sunset | 域级配置 |
| `metric.recycle_bin_days` | 30 | 7–90 | 4.5 回收站保留期 | 域级配置 |
| `metric.deprecate_notice_days` | 30 | 7–180 | 4.5 退役通知期 | 域级配置 |
| `metric.emergency_review_deadline_h` | 24 | 4–72 | 4.5 紧急发布补审时限 | 平台级 |
| `metric.batch_register_limit` | 20 | 5–50 | 4.5 批量注册上限 | 平台级 |
| **质量监控** | | | | |
| `quality.dynamic_baseline.window_days` | 28 | 14–90 | 4.8 动态基线历史窗口 | 指标级覆盖 |
| `quality.dynamic_baseline.sigma` | 3 | 1–5 | 4.8 动态基线 σ 倍数 | 指标级覆盖 |
| `drift_detection.schedule_days` | 7 | 1–30 | 4.7.8 漂移巡检周期 | 域级配置 |
| **权限与合规** | | | | |
| `grant.temp_ttl_days` | 7 | 1–30 | 4.9 临时授权有效期 | 域级配置 |
| `grant.review_cycle_days` | 90 | 30–365 | 4.9 长期权限复审周期 | 平台级 |
| `pii.identification.regex_list` | id_card/phone/email/name | — | 4.15.3 PII 正则匹配模式 | 平台级 |
| `pii.rescan_interval_days` | 7 | 1–30 | R12-09/R12-17 PII 字段定期重扫间隔（天） | 平台级 |
| **审计与留存** | | | | |
| `audit.hot_retention_days` | 180 | 90–365 | 4.10 审计热存天数 | 平台级（合规约束） |
| `audit.cold_archive_enabled` | true | — | 4.10 冷归档开关 | 平台级 |
| `snapshot.hot_retention_days` | 180 | 90–365 | 4.5 快照热存天数 | 平台级（合规约束） |
| `lineage.edge_history_retention_days` | 180 | 90–365 | R10-04 血缘边变更快照热存天数 | 平台级 [R11-16] |
| **消费层** | | | | |
| `api.client_qps_limit` | 20 | 5–200 | 4.11.9 客户端 QPS 上限 | 客户端级 |
| `api.client_daily_limit` | 100000 | 10000–∞ | 4.11.9 客户端日调用上限 | 客户端级 |
| `api.query_timeout_seconds` | 30 | 5–120 | 4.11.4 查询超时 | DataSource 级 |
| `cache.ttl_default_hours` | 24 | 1–168 | 4.11.5 缓存默认 TTL | 指标级覆盖 |
| `search.page_size_default` | 20 | 10–100 | 4.11.2 分页默认页大小 | 平台级 |
| `search.page_size_max` | 100 | 50–500 | 4.11.2 分页最大页大小 | 平台级 |
| **成本与额度** | | | | |
| `api.free_quota_monthly` | 10000 | 0–∞ | R10-01 API 免费额度（次/月/client） | 客户端级 [R11-15] |
| `llm.free_quota_monthly` | 1000 | 0–∞ | R10-01 LLM 解析免费额度（次/月/域） | 域级配置 [R11-15] |
| `cost.budget_alert_threshold_pct` | 80 | 50–95 | R10-01 预算预警阈值（%） | 域级/客户端级 [R11-17] |
| `cost.budget_hard_limit_pct` | 100 | 80–200 | R10-01 预算自动限流阈值（%） | 域级/客户端级 [R11-17] |
| `export.row_limit` | 100000 | 10000–1000000 | 4.11.12 导出行数上限 | 平台级 |
| **备份与运维** | | | | |
| `backup.mysql_full_daily` | true | — | 6.7 MySQL 每日全量备份 | 平台级 |
| `backup.mysql_binlog_enabled` | true | — | 6.7 MySQL binlog 归档 | 平台级 |
| `backup.neo4j_dump_daily` | true | — | 6.7 Neo4j 每日 dump | 平台级 |
| `backup.retention_days` | 7 (MySQL) / 3 (Neo4j/ES) | 1–30 | 6.7 备份保留天数 | 平台级 |
| `consistency.check_daily` | true | — | R8-01 每日对账开关 | 平台级 |
| `consistency.alert_threshold_pct` | 0.1 | 0.01–1.0 | R8-01 对账差异告警阈值% | 平台级 |
| **降级与韧性** | | | | |
| `circuit_breaker.error_rate_pct` | 50 | 10–80 | 4.13.2 熔断错误率阈值 | 依赖级 |
| `circuit_breaker.half_open_timeout_s` | 30 | 10–120 | 4.13.2 半开探测间隔 | 依赖级 |
| `sync.queue_backlog_alert` | 10000 | 1000–100000 | R8-03 同步器积压告警阈值 | 平台级 |
| `sync.queue_backlog_timeout_min` | 30 | 5–120 | R8-03 积压超时告警 | 平台级 |

> **配置变更原则**：① 所有变更入 4.10 审计（含变更前/后 diff）；② 平台级参数由 `platform_admin` 变更；域级参数由 `domain_admin` 变更；③ 合规约束类参数（审计留存/PII 识别）变更须经 `compliance_officer` 复核；④ 变更前可 what-if 预览影响范围（4.9.6）。
