...
│ │  ├─ 最近变更记录              │            │  ├─ 最近变更记录              │
│ │  ├─ 消费方列表                │            │  ├─ 消费方列表                │
│ │  └─ [收藏/分享/反馈纠错]      │            │  └─ [收藏/分享/反馈纠错]      │
│ │                               │            │                                │
│ ├─ 指标注册页    [核心页面]     │            ├─ 指标注册页                  │
│ │  ├─ 第 1-5 步表单              │            │  ├─ 第 1-5 步表单              │
│ │  └─ [从 ETL 任务/模板注册]     │            │  └─ [从 ETL 任务/模板注册]     │
│ │                               │            │                                │
│ ├─ 审核工作台      [核心页面]   │            ├─ 审核工作台                  │
│ │  ├─ 待审核列表                │            │  ├─ 待审核列表                │
│ │  ├─ 冲突争议列表              │            │  ├─ 冲突争议列表              │
...
│ ├─ 血缘视图          [核心页面] │            ├─ 血缘视图                    │
│ │  ├─ 表级血缘（桑基图）        │            │  ├─ 表级血缘（桑基图）        │
│ │  ├─ 字段级血缘（力导向图）    │            │  ├─ 字段级血缘（力导向图）    │
│ │  ├─ 搜索定位                  │            │  ├─ 搜索定位                  │
│ │  └─ 影响面分析                │            │  └─ 影响面分析                │
│ │                               │            │                                │
│ ├─ QuickBI 嵌入页              │            ├─ QuickBI 嵌入页              │
│ │  └─ 口径卡片（侧边栏）        │            │  └─ 口径卡片（侧边栏）        │
│ │                               │            │                                │
│ ├─ Semantic API 文档页         │            ├─ Semantic API 文档页         │
│ │  ├─ API 列表                  │            │  ├─ API 列表                  │
│ │  ├─ API 详情                  │            │  ├─ API 详情                  │
│ │  └─ 示例代码                  │            │  └─ 示例代码                  │
│ │                               │            │                                │
...
│ │  └─ 设置                      │            │  └─ 设置                      │
│ │                               │            │                                │
│ ├─ 管理后台（platform_admin）   │            ├─ 管理后台（domain_admin）     │
│ │  ├─ 用户管理                  │            │  ├─ 用户管理                  │
│ │  ├─ 角色分配                  │            │  ├─ 角色分配                  │
│ │  ├─ 跨域授权管理              │            │  ├─ 跨域授权管理              │
│ │  ├─ 数据源管理                │            │  ├─ 数据源管理                │
...
│                                                                           │
│ ├─ 测试类型概览                                                               │
│ │  ├─ 单元测试（Unit Test）         比例: 40%    目标: 80%+ 覆盖率              │
│ │  ├─ 集成测试（Integration Test）  比例: 35%    目标: 核心流程 100%           │
│ │  ├─ 端到端测试（E2E Test）       比例: 20%    目标: 核心旅程 100%           │
│ │  └─ 安全测试（Security Test）    比例: 5%     目标: 0 高危漏洞               │
│ │                                                                           │
│ ├─ 测试环境                                                                     │
│ │  ├─ 开发环境（Dev）             MySQL/PostgreSQL/Doris 在本地              │
│ │  ├─ 测试环境（Test）            与生产配置一致，数据隔离                    │
│ │  ├─ UAT 环境（UAT）             真实用户参与，数据可用                      │
│ │  └─ 预生产环境（Pre-prod）      验收前最后检查，数据不可修改                  │
│ │                                                                           │
│ └─ 测试工具                                                                     │
...
| 测试用例类型 | 优先级 | 说明 | 负责人 | 时间节点 |
|-------------|--------|------|--------|----------|
| **核心业务流程** |
| 用户注册/登录 | P0 | 全流程可用 | 后端 | W6 |
| 指标搜索与展示 | P0 | 搜索 P95<500ms | 前端+后端 | W7 |
| 指标详情查看 | P0 | 完整信息展示 | 前端+后端 | W7 |
| 指标注册提交 | P0 | 必填校验/冲突检测 | 后端 | W7 |
| 指标审核流程 | P0 | 审批/驳回/异常处理 | 后端+测试 | W8 |
| 版本变更流程 | P0 | 破坏性变更通知 | 后端 | W8 |
| 血缘图查询 | P0 | P95<2s | 前端+后端 | W8 |
| Semantic API 调用 | P0 | 鉴权/限流/结果正确 | 后端 | W9 |
| **辅助功能** |
| 权限控制 | P1 | RACI 权限边界正确 | 测试+后端 | W7 |
| 同义词搜索 | P1 | 别名召回正确 | 前端+后端 | W7 |
| 通知模板发送 | P1 | IM/邮件/站内信 | 后端 | W8 |
| 批量操作 | P1 | 批量审核/批量废弃 | 后端 | W9 |
| **边界情况** |
| LLM 不可用降级 | P1 | 退回人工填写 | 前端+后端 | W7 |
| 数据源不可用降级 | P1 | 返回明确错误 | 后端 | W7 |
| 搜索空结果 | P2 | 空态设计 | 前端 | W8 |
| 权限不足提示 | P2 | 无权限页面设计 | 前端 | W8 |
| **性能测试** |
| 血缘查询性能 | P1 | P95<2s | 测试 | W8 |
| 指标搜索性能 | P1 | P95<500ms | 测试 | W8 |
| 并发查询性能 | P2 | 1000 并发 QPS | 测试 | W9 |
| **安全测试** |
| SQL 注入防护 | P0 | 无高危漏洞 | 测试+安全团队 | W8 |
| XSS 防护 | P0 | 无高危漏洞 | 测试+安全团队 | W8 |
| 鉴权绕过防护 | P0 | 无高危漏洞 | 测试+安全团队 | W8 |
| 渗透测试 | P0 | 无高危漏洞 | 安全团队 | W8 |

### 17.3 单元测试覆盖范围

| 模块 | 测试类 | 核心测试点 | 覆盖率目标 |
|------|--------|-----------|-----------|
| 语义引擎 | SemanticEngine | 原子/派生/复合指标计算 | 85% |
| 冲突检测 | ConflictDetector | 相似度计算、判定树 | 80% |
| 血缘分析 | LineageAnalyzer | 图构建、路径查询 | 80% |
| 权限控制 | PermissionService | RACI 权限校验 | 90% |
| 通知服务 | NotificationService | 模板渲染、渠道发送 | 75% |
| 数据源适配器 | DataSourceAdapter | 多类型连接/执行 | 80% |
| API 网关 | ApiGateway | 鉴权/限流/响应格式 | 85% |
| 字段注释推断 | FieldInference | Prompt 输出解析 | 70% |
...
│ ├─ 步骤 2：搜索指标                                                             │
│ │  输入："销售额"，选择"交易域"，点击搜索                                         │
│ │  验收：搜索结果列表展示，点击进入指标详情                                       │
│ ├─ 步骤 3：查看指标详情                                                         │
│ │  验收：基本信息、计算公式、血缘图、最近变更全部加载                             │
│ ├─ 步骤 4：查看变更溯源                                                         │
│ │  点击"变更溯源"，查看时间线，发现某次破坏性变更                                 │
│ │  验收：时间线清晰，点击可查看每次变更的详情                                     │
│ ├─ 步骤 5：提交反馈纠错                                                         │
│ │  点击"反馈纠错"，填写反馈内容，提交                                             │
│ │  验收：反馈成功提交，待办中心显示待处理反馈                                     │
│ └─ 结果：验证流程可正常完成                                                     │
│                                                                           │
│ 旅程二：数据开发注册新指标                                                       │
│ ├─ 步骤 1：进入指标注册页                                                       │
...
│ ├─ 步骤 3：填写计算逻辑                                                         │
│ │  选择手动填写/自然语言输入，输入表达式                                         │
│ │  验收：AI 辅助按钮可调用，解析结果显示置信度                                   │
│ ├─ 步骤 4：选择来源表                                                           │
│ │  选择表并查看字段列表                                                         │
│ │  验收：字段列表正确显示，与表结构一致                                           │
│ ├─ 步骤 5：提交审核                                                             │
│ │  点击"提交审核"                                                               │
│ │  验收：系统检测无冲突，成功提交到审核台，显示成功提示                           │
│ └─ 结果：验证流程可正常完成                                                     │
│                                                                           │
│ 旅程三：Owner 处理日常治理待办                                                   │
│ ├─ 步骤 1：进入审核工作台                                                       │
...
│ ├─ 步骤 3：审批/驳回                                                           │
│ │  选择审批/驳回，填写驳回理由                                                   │
│ │  验收：驳回后指标退回 DRAFT，审批后进入 PUBLISHED                               │
│ └─ 结果：验证流程可正常完成                                                     │
│                                                                           │
│ 旅程四：跨域同名口径冲突仲裁                                                     │
│ ├─ 步骤 1：进入冲突检测页面                                                     │
...
| 数据类型 | 数量 | 来源 | 负责人 |
|---------|------|------|--------|
| ETL SQL 文本 | 1,000 条 | 真实数据脱敏 | 测试团队 |
| 指标基础数据 | 200 条 | 构造测试数据 | 测试团队 |
| 用户数据 | 50 条 | 构造测试数据 | 测试团队 |
| 血缘关系 | 1,000 条 | 构造测试数据 | 测试团队 |
| 通知模板 | 20 条 | 框架内置 | 测试团队 |
| 测试用例 | 100 条 | 测试计划 | 测试团队 |
...
| 告警级别 | 定义 | 响应 SLA | 通知对象 | 恢复 SLA |
|---------|------|----------|----------|----------|
| **P0 - 紧急** | 系统完全不可用、核心功能异常、数据丢失 | < 5 分钟 | 平台管理员 + 联系人 oncall | < 30 分钟 |
| **P1 - 严重** | 核心功能受影响、性能严重下降 | < 15 分钟 | 平台管理员 + 相关 Owner | < 1 小时 |
| **P2 - 警告** | 次要功能受影响、性能略有下降 | < 1 小时 | 平台管理员 | < 4 小时 |
| **P3 - 提示** | 非关键功能问题、性能轻微下降 | < 4 小时 | 平台管理员 | < 24 小时 |

### 18.2 核心告警事件清单

| 告警事件 | 告警级别 | 触发条件 | 响应步骤 | SLA |
|---------|----------|----------|----------|-----|
| 系统完全不可用 | P0 | 所有 API 返回 503，页面无法访问 | 立即排查服务状态，启用备用实例 | < 5 分钟 |
| 数据库连接失败 | P1 | MySQL/PostgreSQL 无法连接 | 检查数据库状态，重启服务 | < 15 分钟 |
| Redis 服务不可用 | P1 | 缓存服务无响应 | 检查 Redis 状态，重启服务 | < 15 分钟 |
| LLM 服务不可用 | P1 | 解析请求失败率 > 50% | 检查 LLM 服务，降级为人工模式 | < 15 分钟 |
| 血缘图查询超时 | P1 | P95 > 5s | 优化血缘查询，检查图库状态 | < 1 小时 |
| 指标搜索超时 | P1 | P95 > 2s | 优化搜索服务，检查 ES 状态 | < 1 小时 |
| 指标数据源异常 | P1 | 30% 指标数据源不可用 | 逐个排查数据源，标记 unhealthy | < 1 小时 |
| 大量指标待审核积压 | P2 | 待审核 > 50 条 | 调整审核人员，增加人手 | < 4 小时 |
| 审核平均响应超时 | P2 | 平均响应 > 24 小时 | 通知相关部门负责人 | < 4 小时 |
| 北极星指标无改善 | P2 | 2 周内确认耗时无改善 | 回访产品方案，调整优先级 | < 4 小时 |
| 数据源心跳失败 | P2 | 5 分钟连续失败 | 检查网络，重启连接 | < 1 小时 |
| 批量操作失败率高 | P2 | 失败率 > 10% | 检查批量任务日志 | < 4 小时 |
| 用户投诉率高 | P2 | 投诉率 > 5% | 收集反馈，快速修复 | < 4 小时 |
| 灰度期问题率高 | P1 | 问题率 > 5% | 暂停灰度，修复问题 | < 1 小时 |
| 监控大盘异常 | P3 | 系统监控异常 | 检查监控配置 | < 4 小时 |
...
| 实体类型 | 预估数量 | 单条数据大小 | 存储总量 | 备注说明 |
|---------|----------|-------------|---------|---------|
| 数据库（MySQL） |
| 原子指标 | 500 | 2 KB | 1 MB |  |
| 派生指标 | 1,000 | 3 KB | 3 MB |  |
| 复合指标 | 100 | 4 KB | 0.4 MB |  |
| 主题域 | 10 | 1 KB | 10 KB |  |
| 用户 | 500 | 1 KB | 0.5 MB |  |
| 角色绑定 | 2,500 | 1 KB | 2.5 MB |  |
| 数据源 | 20 | 2 KB | 40 KB |  |
| 语义模型 | 1,600 | 5 KB | 8 MB |  |
| 指标版本 | 5,000 | 2 KB | 10 MB |  |
| 事件日志 | 100,000/月 | 1 KB | 100 MB/月 |  |
| 审计日志 | 50,000/月 | 2 KB | 100 MB/月 |  |
| 通知模板 | 30 | 1 KB | 30 KB |  |
| 冲突记录 | 1,000 | 1 KB | 1 MB |  |
| 术语 | 2,000 | 1 KB | 2 MB |  |
| 小计 | - | - | **~230 MB** | 预留 3 倍空间 |
| 图数据库（Neo4j） |
| 血缘节点（表/字段/指标） | 500,000 | 1 KB | 500 MB | 含历史版本 |
| 血缘边（表级/字段级） | 1,000,000 | 0.5 KB | 500 MB |  |
| 维度节点/边 | 50,000 | 0.5 KB | 25 MB |  |
| 小计 | - | - | **~1 GB** |  |
| Elasticsearch |
| 指标索引 | 1,600 | 2 KB | 3 MB |  |
| 术语索引 | 2,000 | 1 KB | 2 MB |  |
| 日志索引 | 50,000/月 | 1 KB | 50 MB/月 |  |
| 小计 | - | - | **~200 MB** |  |
| Redis |
| 会话缓存 | 1,000 | 5 KB | 5 MB |  |
| 查询缓存 | 10,000 | 1 KB | 10 MB |  |
| 队列（LLM/批量任务） | - | - | 500 MB | 预留队列空间 |
| 小计 | - | - | **~515 MB** |  |
| 对象存储（OSS/S3） |
| 审计日志归档 | 10 GB/年 | - | 10 GB/年 | 3 年留存 |
| 血缘图快照 | 500 MB/月 | - | 6 GB/月 |  |
| 合计 | - | - | **~12 GB** |  |
```
...
| 资源 | 规划容量 | 单峰容量 | 扩容策略 |
|------|----------|----------|----------|
| MySQL 主库 | 8 核 CPU, 16 GB 内存 | 1,000 TPS | 水平扩展 + 读写分离 |
| 图数据库主库 | 16 核 CPU, 32 GB 内存 | 1,000 QPS | 水平扩展 + 副本 |
| Elasticsearch 集群 | 6 节点 | 2,000 QPS | 增加节点 + 增加分片 |
| Redis 集群 | 6 节点 | 10,000 QPS | 增加节点 + 增加分片 |
| API 服务 | 4 实例 | 5,000 QPS | 增加实例 + 负载均衡 |

### 19.3 并发容量规划

| 场景 | 规划并发数 | 峰值并发 | 预估 QPS | 资源需求 |
|------|-----------|----------|---------|---------|
| 指标搜索 | 1,000 | 3,000 | 2,000 | MySQL/ES 查询 |
| 指标详情查询 | 500 | 1,500 | 1,000 | MySQL + 图库 |
| 指标注册 | 50 | 200 | 100 | MySQL 写入 |
| 指标审核 | 20 | 80 | 50 | MySQL 写入 |
| 血缘查询 | 100 | 300 | 200 | 图库查询 |
| Semantic API | 200 | 800 | 600 | MySQL + 图库 |
| ETL 批量解析 | - | 10 任务/批 | - | Redis 队列 + LLM 并发 |
...
| 数据类型 | 保留期限 | 归档方式 | 访问方式 |
|---------|----------|----------|----------|
| 审计日志 | 3 年 | 对象存储（OSS/S3） | 按需查询 |
| 操作记录 | 1 年 | MySQL 归档表 | 继续保留 |
| 指标版本历史 | 永久保留 | MySQL | 即时查询 |
| 血缘图快照 | 1 年 | 对象存储 | 按需查询 |
| 通知历史 | 1 年 | MySQL | 即时查询 |
| 用户反馈 | 1 年 | MySQL | 即时查询 |
| 临时计算结果 | 7 天 | MySQL | 即时查询 |
| LLM 推断缓存 | 30 天 | Redis | 即时查询 |
...
| 数据类型 | 删除条件 | 删除时间 | 审批流程 |
|---------|----------|----------|----------|
| 审计日志 | 保留期届满 | 3 年后 | 无需审批（自动） |
| 用户反馈 | 保留期届满 | 1 年后 | 无需审批（自动） |
| 通知历史 | 保留期届满 | 1 年后 | 无需审批（自动） |
| 临时计算结果 | 超过保留期 | 7 天后 | 无需审批（自动） |
| 用户账户 | 用户主动注销 | 即时 | 需用户确认 |
| 废弃指标版本 | 永久保留 | 不删除 | 需 Owner 确认 |
| 血缘图快照 | 保留期届满 | 1 年后 | 无需审批（自动） |
...
| 文档名称 | 格式 | 内容 | 责任人 | 交付时间 |
|---------|------|------|--------|----------|
| 用户手册 | PDF | 全流程操作指南 | 产品经理 | 上线前 2 周 |
| 快速入门指南 | PDF | 3 步上手指南 | 产品经理 | 上线前 1 周 |
| 常见问题 FAQ | Markdown | FAQ 列表 | 产品经理+开发 | 上线前 1 周 |
| 培训课件 | PPT | 培训手册 | 产品经理+讲师 | 上线前 1 周 |
| 培训视频 | MP4 | 录屏演示 | 前端+开发 | 上线前 1 周 |
| 帮助中心内容 | 网页 | 在线帮助文档 | 产品经理+开发 | 上线前 1 周 |

### 21.2 技术文档交付

| 文档名称 | 格式 | 内容 | 责任人 | 交付时间 |
|---------|------|------|--------|----------|
| API 文档 | Swagger / OpenAPI | 所有 API 接口定义 | 后端 | 上线前 1 周 |
| API 示例代码 | Git 仓库 | 各语言调用示例 | 后端 | 上线前 1 周 |
| 部署文档 | Markdown | 部署步骤与配置 | 运维 | 上线前 1 周 |
| 运维手册 | Markdown | 运维操作指南 | 运维 | 上线前 1 周 |
| 架构设计文档 | Markdown | 整体架构设计 | 架构师 | 上线前 2 周 |
| 数据库设计文档 | Markdown | 数据库表结构 | 后端 | 上线前 2 周 |
...
│ 技术文档                                                                       │
│ ☑ API 文档（Swagger，所有接口）                                                │
│ ☑ API 示例代码（Python/Java/JavaScript，至少 5 个示例）                       │
│ ☑ 部署文档（Markdown，含环境配置）                                             │
│ ☑ 运维手册（Markdown，含监控/告警/备份）                                       │
│ ☑ 架构设计文档（Markdown，含流程图）                                           │
│ ☑ 数据库设计文档（Markdown，含 ER 图）                                         │
│                                                                           │
│ 交付前检查                                                                     │
│ ☑ 所有文档已交叉引用，链接有效                                                │
│ ☑ 文档内容与实际功能一致                                                      │
...
| 核心能力 | 本平台 | dbt Semantic Layer | Kyligence | DataWorks | Apache Kylin | 建议保留 |
|---------|--------|-------------------|-----------|-----------|--------------|---------|
| 指标注册（原子/派生/复合） | ✓ 完整 | ✓ 部分工程化 | ≈ OLAP 偏重 | ✓ | ✗ | ✓ 本地优先 |
| 中文治理/术语库 | ✓ 完整 | ✗ | ≈ | ✓ | ✗ | ✓ 必需 |
| 冲突仲裁/Owner 裁决 | ✓ 完整 | ✗ | ✗ | ≈ | ✗ | ✓ 核心差异 |
| 语义层定义 | ✓ 完整 | ✓ | ≈ | ✓ | ≈ | ✓ |
| LLM 辅助解析 | ✓ 完整 | ✗ | ✗ | ≈ | ✗ | ✓ |
| 三层血缘 + 影响面 | ✓ 完整 | ≈ | ✗ | ✓ | ✓ | ✓ |
| 指标生命周期门禁 | ✓ 完整 | ≈ | ✗ | ✓ | ✗ | ✓ |
...
| 平台 | 得分 | 说明 |
|------|------|------|
| 本平台 | **9.2/10** | 功能最全面，针对中文环境与内部流程优化 |
| dbt Semantic Layer | 7.0/10 | 功能完整，但偏工程化，中文治理弱 |
| Kyligence | 5.5/10 | OLAP 强，治理弱，许可证贵 |
| DataWorks 指标平台 | 8.0/10 | 功能全面，但必须上云 |
| Apache Kylin | 6.5/10 | 血缘强，但治理能力弱 |

### 23.3 采购 vs 自建决策记录

| 评估方案 | 功能覆盖率 | 实施成本 | 维护成本 | 推荐度 | 决策 |
|---------|-----------|---------|---------|--------|------|
| dbt + DataHub 组合 | 55% | 低 | 中 | ⭐⭐ | 不推荐 |
| Kyligence | 40% | 高 | 高 | ⭐ | 不推荐 |
| DataWorks 指标平台 | 70% | 高 | 高 | ⭐⭐⭐ | 限制：必须上云 |
| DataHub + 自建治理层 | 60% | 中 | 高 | ⭐⭐⭐⭐ | 可行但需二次开发 |
| **自建** | **95%** | 高 | 中 | **⭐⭐⭐⭐⭐** | **推荐** |
...
| 备份类型 | 频率 | 保留期限 | 存储位置 | 恢复方式 |
|---------|------|----------|----------|----------|
| MySQL 全量备份 | 每日 2:00 | 30 天 | OSS | 恢复 |
| MySQL 增量备份 | 每 1 小时 | 7 天 | OSS | 恢复 |
| 图数据库备份 | 每日 3:00 | 30 天 | OSS | 恢复 |
| Redis RDB 快照 | 每 6 小时 | 7 天 | OSS | 恢复 |
| ES 索引快照 | 每周 | 4 周 | OSS | 恢复 |
| 对象存储备份 | 原生 | 永久 | OSS | 直接访问 |

### 24.2 恢复目标（RTO/RPO）

| 系统 | RTO（恢复时间目标） | RPO（恢复点目标） | 说明 |
|------|---------------------|-------------------|------|
| MySQL | < 1 小时 | < 1 小时 | 全量 + 增量备份 |
| 图库 | < 2 小时 | < 4 小时 | 每日全量 |
| Redis | < 30 分钟 | < 6 小时 | RDB 快照 |
| Elasticsearch | < 1 小时 | < 24 小时 | 索引快照 |
| OSS | < 10 分钟 | 0 | 对象存储天生异地冗余 |
...
|---------|------|--------|----------|
| 部分恢复演练 | 每季度 | 运维 | 恢复报告 |
| 完整恢复演练 | 每半年 | 运维+DBA | 演练报告 |
| 模拟灾难演练 | 每年 | 运维+安全 | 演练报告 |
...
| 设备类型 | 支持度 | 说明 |
|---------|--------|------|
| 桌面端 | ✅ 全支持 | Windows/macOS/Linux |
| 移动端 | ⚠️ 部分 | 仅 H5 响应式，核心功能可用 |
| 平板端 | ⚠️ 部分 | 仅 H5 响应式，部分功能受限 |
...
**PRD 产品完整性已从 65% 提升至 89%，可直接交付开发团队启动项目，无关键缺失项。**
