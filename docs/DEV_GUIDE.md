...
- **README 同步（面向用户/外部视角）**：以下变更须同步更新 `README.md` 对应章节，保持与项目最新状态一致：
  - **新增/重命名/下线前端页面或领域模块** → 更新「核心能力」与「领域模块」清单（模块清单以 `docs/module-status.yaml` 为准，二者不得脱节）。
  - **技术栈/存储/依赖变更**（如新增组件、更换搜索引擎、引入新中间件） → 更新「技术栈」分层表。
  - **部署方式/端口/镜像名/启动流程变更** → 更新「快速开始」与 `docs/QUICKSTART-DOCKER.md` 引用。
  - **新增关键文档**（对接指南/部署手册/产品说明等） → 更新「关键文档」索引。
  - **测试体系变更**（新增测试类型/门禁增减） → 更新「测试」矩阵。
  - **Git 远程/镜像推送策略变更** → 更新「Git 多远程」速查。
  - README 不在 `contract_check.py` 自动校验范围，由 PR Review Checklist（§6.2 第 7 项）与独立复核（§1.5）强制核对。
...
7. **文档同步**：改接口→§3 同步；改表→§4.1 同步；改状态机→§12.x 同步；PR 填 TD 影响章节；**README 同步**：新增/重命名/下线页面或领域模块、技术栈/部署/关键文档/测试体系/Git 远程变更，须核对 README 对应章节（清单见 §4 第 71-78 行）。
...
## 7a. 指标类型 OneData 语义共识（强制遵循）

指标类型定义以阿里 OneData 理论为准（《大数据之路》第 4 章），**本节为唯一口径**，任何实现/文案/文档不得与之冲突。历史曾出现「原子指标 = 逻辑度量 + 业务限定 + 统计周期」的错误表述，与 OneData 冲突，已于 2026-08-26 经产品确认统一纠正（见 commit `526d7d9`）。

| 类型 | OneData 权威定义 | 系统落地 |
|---|---|---|
| **原子指标** | **业务过程 + 度量**（业务定义中不可再拆分的指标；**不含**统计粒度/业务限定/时间周期） | `measure_catalog`（逻辑度量）承载「原子 = 裸度量」层；`metric.atomic` 保留为「**逻辑度量 + 基础统计粒度（日）**」的变体便捷标签 |
| **派生指标** | **原子指标 + 统计粒度 + 业务限定 + 时间周期** | `metric.derived`；**依赖指标可选**（纯周期/业务限定派生可不依赖），可携带挂载实体（结果落表） |
| **复合指标** | 多个派生指标**四则运算/比率** | `metric.composite`；**须声明依赖指标 + 计算表达式** |

**关键约定（务必牢记，避免再次出现概念误解）：**

1. **原子指标不含业务限定与时间周期**——业务限定、时间周期一律归派生。凡是 SQL 解析出「带业务限定 / 非日周期」的指标，类型判定为**派生**（周期驱动，见 `backend/app/services/semantic/sql_split.py` / `auto_fill.py::_infer_type`）。
2. **日粒度（day）例外**：作为原子指标的**基础统计粒度**不视为派生周期；月/周/季/年/小时等非日周期 → 派生。
3. **派生依赖可选**：纯周期/业务限定派生（如「本月活跃医生数」）可不依赖其他指标；复合仍强制依赖。
4. **展示/文案统一**：类型说明以 `frontend/src/utils/enums.ts`（`METRIC_TYPE_DESC`）、`frontend/src/pages/MetricCreate.tsx`（`TYPE_HINTS`）为准，禁止再写「原子 = 逻辑度量 + 业务限定 + 统计周期」类表述。
5. **权威文档**：`docs/指标设计以及界限说明.md`（§1.1 定义表已同步本共识）；改指标类型相关代码/文档前先核对本节。
