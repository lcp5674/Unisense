"""资产地图真实 MySQL 集成测试（TD §12.11 / FR-18）。

验证 AssetMapService 在真实 schema 上可正确执行聚合查询（catalog/metric/orphan），
返回结构正确且无异常。schema 由 Alembic 迁移建表。
"""
...
async def test_catalog_summary_executes(db_env) -> None:
    async with db_env["session_factory"]() as session:
        result = await AssetMapService(session).catalog_summary()
        assert isinstance(result, dict)
        assert "total" in result


async def test_metric_summary_executes(db_env) -> None:
    async with db_env["session_factory"]() as session:
        result = await AssetMapService(session).metric_summary()
        assert isinstance(result, dict)
