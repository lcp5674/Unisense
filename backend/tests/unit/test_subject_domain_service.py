...
class TestAutoGeneratedCode:
    ...
    async def test_pure_chinese_falls_back_to_domain(self, svc) -> None:
        """纯中文名无 ASCII → 根域回退为 domain。"""
        svc._repo.code_exists = AsyncMock(return_value=False)
    ...
    async def test_child_chinese_falls_back_to_parent_sub(self, svc) -> None:
        """子域纯中文名 → 父域_sub。"""
        parent = MagicMock()
        parent.id = 5
        parent.code = "sales"
