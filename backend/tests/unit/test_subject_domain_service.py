...
class TestAutoGeneratedCode:
    ...
    async def test_pure_chinese_generates_pinyin(self, svc) -> None:
        """纯中文名 → 拼音 slug（如 销售 → xiaoshou）。"""
        svc._repo.code_exists = AsyncMock(return_value=False)
        result = await self._create(svc, name="销售")
        assert result.code == "xiaoshou"

    async def test_mixed_chinese_ascii_keeps_both(self, svc) -> None:
        """中英混合名 → 拼音与 ASCII 共存（销售订单GMV → xiaoshoudingdan_gmv）。"""
        svc._repo.code_exists = AsyncMock(return_value=False)
        result = await self._create(svc, name="销售订单GMV")
        assert result.code == "xiaoshoudingdan_gmv"
    ...
    async def test_child_chinese_generates_pinyin(self, svc) -> None:
        """子域纯中文名 → 拼音并带父域前缀（订单 + sales → sales_dingdan）。"""
        parent = MagicMock()
        parent.id = 5
        parent.code = "sales"
        parent.level = 1
        ...
        assert result.code == "sales_dingdan"
