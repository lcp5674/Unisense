...
class TestBuildKey:
    def test_explicit_key_derived_via_sha256(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """显式配置 UNISENSE_FERNET_KEY 时经 SHA-256 → base64url 派生。"""
        monkeypatch.setenv("UNISENSE_FERNET_KEY", "my-custom-fernet-key")
        key = _build_key()
        expected = base64.urlsafe_b64encode(hashlib.sha256(b"my-custom-fernet-key").digest())
        assert key == expected
