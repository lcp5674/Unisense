...
class TestDimensionCRUD:
    ...
    async def test_list_dimensions_visible_domain_admin_no_domain_global(
        self, repo, session
    ) -> None:
        ...
        stmt = _rows_stmt(session)
        # 不限域：不附加 status/domain/owner 可见性条件
        assert "PUBLISHED" not in stmt
        assert "dimension.owner_id = 2" not in stmt
        assert "dimension.domain IN" not in stmt
