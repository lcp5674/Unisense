"""采集领域可观测测试（对齐 gateways observability）。
...
class _FakeListSvc:
    ...
    async def list_sources(self, **kw: object) -> tuple[list, int]:
        return ([], 0)


async def test_collector_response_contains_trace_id(owner_client, monkeypatch):
    monkeypatch.setattr(collector_api, "CollectorService", _FakeListSvc)
    resp = await owner_client.get("/api/v1/data-sources", headers={"Authorization": "Bearer x"})
