"""recommend 混沌测试（对齐 gateways chaos，TD §15）。"""
...
async def test_terms_returns_503_on_db_failure(
    reader_client: httpx.AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    async def boom(self: RecommendService, limit: int = 20) -> object:
        raise ExternalDependencyError("db 不可达")

    monkeypatch.setattr(RecommendService, "recommend_terms", boom)
