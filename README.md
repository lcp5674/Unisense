...
# ── 第 1.5 步：binlog 独立卷权限初始化（compose 已自动，无需手工）────────
#   mysql 以 uid=999 写 binlog 卷 /var/lib/mysql-binlog（独立卷，官方镜像 entrypoint
#   不自动 chown 自定义挂载点）。docker-compose.yml 已内置一次性 mysql-init 服务：
#   mysql 启动前自动 chown 该卷（depends_on service_completed_successfully，幂等），
#   `docker compose up -d` 全自动，**无需执行任何手工命令**。
#   仅当不使用本 compose（裸机/自编编排部署）时才需手动初始化一次：
