...
// OneData 语义说明（类型展示 Tooltip，变体口径）：原子 = 逻辑度量 + 基础统计粒度（日），
// 不含业务限定与时间周期；派生 = 原子 + 业务限定 + 时间周期（月/周/季/年，依赖可选）；
// 复合 = 多指标四则运算/比率。
export const METRIC_TYPE_DESC: Record<string, string> = {
  atomic: "OneData 原子层：通用逻辑度量 + 基础统计粒度（日）。可复用、不绑定业务限定与时间周期（变体口径：把日粒度算进原子的便捷标签）；可关联逻辑度量目录统一管理格式/单位。",
  derived: "OneData 派生层：原子指标 + 业务限定 + 时间周期（月/周/季/年）。如「本月活跃医生数」= 活跃医生数 + 业务限定 + 月周期；依赖指标可选，可携带挂载实体（结果落表）。",
