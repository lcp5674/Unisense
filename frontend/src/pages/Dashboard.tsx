...
export function Dashboard() {
  ...
  // 指标待审核（评审动作告警）按「可审」口径收敛（TD §13）：后端按当前用户角色与评审
  // 指派计算 assigned_review（platform_admin=全量；domain_admin=指派给我/本域/未指派兜底；
  // reviewer=指派给我/本域；无评审能力=0）——与审批中心「待我审」视图完全一致，
  // 只展示需要自己审批的条目，其他用户的审批不在此展示（本人名下审核中状态已由
  // Owner 责任分布体现，属资产状态而非评审队列）。
  const reviewCount = data.assigned_review ?? 0;
